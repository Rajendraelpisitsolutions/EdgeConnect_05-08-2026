...
public sealed class MainForm : Form
{
    ...
    private readonly RadioButton _modeLoad = new() { Text = "Load License", AutoSize = true };
    ...
    public MainForm()
    {
        Text = "Elpis EdgeConnect — License Generator";
        ...
        _product.Items.AddRange(new object[] { "Elpis EdgeConnect", "EREMOS V2" });
        _product.SelectedIndex = 0;
        _edition.Items.AddRange(new object[] { "Starter", "Professional", "Enterprise" });
        ...
        // Expiry: date picker + quick-duration presets + a "valid for N days" hint.
        ...
        expiryRow.Controls.Add(MakePresetButton("1Y", 1));
        expiryRow.Controls.Add(MakePresetButton("2Y", 2));
        expiryRow.Controls.Add(MakePresetButton("3Y", 3));
        ...
        {
            Text = "Submit — create license.json",
            AutoSize = true,
    ...
    private void BuildLoadPanel()
    {
        ...
        var title = new Label
        {
            Text = "Load a previously issued license",
            AutoSize = true,
            Font = new Font("Segoe UI", 11f, FontStyle.Bold),
            Margin = new Padding(0, 0, 0, 12),
        };
    ...
    {
        var edition = ParseEdition(_edition.SelectedItem as string);
        var restrictsSources = LicenseEditionCatalog.RestrictsSources(edition);

        var offeredSources = new List<(string, string)>();
