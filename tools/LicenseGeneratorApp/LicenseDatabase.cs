...
-- Catalogue of product names offered in the New License form. Separate from
-- IssuedLicenses on purpose: a product has to be selectable BEFORE any license
-- has been issued for it, so it cannot be derived from issued rows the way the
-- update-flow drill-down does.
...
";
        ...
        // Seed the two names the form previously hard-coded, so an existing
        // database keeps offering them and a fresh one starts populated.
        // INSERT OR IGNORE keeps this idempotent across launches, and never
        // resurrects a name the operator has since removed... it only adds when
        // the table is genuinely missing the row.
        using var seed = conn.CreateCommand();
        seed.CommandText = @"
INSERT OR IGNORE INTO Products (Name, CreatedAtUtc) VALUES ('Elpis EdgeConnect', $now);
INSERT OR IGNORE INTO Products (Name, CreatedAtUtc) VALUES ('EREMOS V2', $now);
";
    ...
    /// <summary>
    /// Every product name offered in the New License form's Product dropdown.
    /// </summary>
    public static List<string> GetProductCatalog() => QueryColumn(
        "SELECT Name FROM Products ORDER BY Name",
        _ => { });

    /// <summary>
    /// Adds a product name to the catalogue. Returns <c>false</c> when the name
    /// is already present (compared case-insensitively, so "Elpis EdgeConnect"
    /// and "elpis edgeconnect" cannot both exist and confuse the dropdown).
    /// </summary>
    public static bool AddProduct(string name)
    {
        ...
        exists.CommandText = "SELECT COUNT(*) FROM Products WHERE Name = $n COLLATE NOCASE";
        exists.Parameters.AddWithValue("$n", trimmed);
        ...
        insert.CommandText = "INSERT INTO Products (Name, CreatedAtUtc) VALUES ($n, $now)";
        insert.Parameters.AddWithValue("$n", trimmed);
