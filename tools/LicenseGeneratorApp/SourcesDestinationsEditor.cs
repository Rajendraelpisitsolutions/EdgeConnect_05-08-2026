...
public sealed class SourcesDestinationsEditor : UserControl
{
    ...
    private void Rebuild(TableLayoutPanel grid, List<(string Display, string Key)> options, int maxRows, int maxColumns)
    {
        ...
        // Fill row-first: one item per column up to maxColumns, then wrap. This
        // keeps a small offering (e.g. CNC Pro's three sources) on a SINGLE row
        // instead of packing them into 2 columns × 2 rows. Larger offerings still
        // wrap within maxColumns (maxRows kept as a sizing hint for the card).
        _ = maxRows;
        var columns = Math.Max(1, Math.Min(maxColumns, options.Count));
