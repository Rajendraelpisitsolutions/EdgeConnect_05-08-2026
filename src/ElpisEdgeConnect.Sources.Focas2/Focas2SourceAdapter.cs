...
public sealed class Focas2SourceAdapter : ISourceAdapter, ISourceRetirement
{
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// Deliberately does NOT dial the controller. The first PollAsync calls
    /// EnsureConnected + EnsureSystemInfo itself, so an eager connect here
    /// bought nothing and its outcome was discarded either way — both the
    /// success and failure paths ended at AdapterState.Running.
    ///
    /// What it cost: StartAsync is awaited by SourceSupervisor.AddAsync, which
    /// the hot-reload coordinator awaits per source while applying a config.
    /// Against a device that is not yet wired up — the normal case when an
    /// operator configures ahead of installation — the connect burned the whole
    /// retry budget (maxConnectRetries 5 x timeoutSeconds 10 = up to 50s) before
    /// giving up, so every Save in the onboarding flow blocked until the apply
    /// endpoint's ReloadOutcomeWaitMs cap (10s) expired and returned InProgress.
    /// Connecting is now the poll loop's job alone, which is where the backoff,
    /// the retry accounting and the operator-facing fault reporting already live.
    /// </remarks>
    public Task StartAsync(CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            "Focas2SourceAdapter '{Id}' started; connecting on first poll.", _instanceId);
        return Task.CompletedTask;
    }
