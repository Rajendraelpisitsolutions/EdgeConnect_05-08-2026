...
public sealed class MqttSinkAdapter : ISinkAdapter
{
        ...
        CancellationToken ct)
    {
        ...
        {
            // PerTag publishes a BARE SCALAR — the topic is the identity and the
            // payload is the value, with nowhere to carry quality. (Batch mode
            // does carry it: see MqttPayloadSerializer, which writes "quality"
            // and "qualityReason".) So a Bad point here became an EMPTY payload
            // logged as "Result: OK", and a downstream consumer could not tell a
            // dead source from a real reading while the sink looked healthy.
            //
            // Suppress rather than publish. Adding a quality field would change
            // the EREMOS V2 topic contract for every existing consumer; dropping
            // the message keeps that contract intact and lets MQTT's own
            // semantics apply — the last good retained value stands, and a
            // consumer watching timestamps still sees the data go stale.
            //
            // Counted as `suppressed`, NOT `rejected`: rejected forces
            // Success=false, which holds the buffer cursor and re-delivers the
            // batch forever. A Bad point is not a delivery failure and must not
            // stall the buffer.
            if (point.Quality == DataQuality.Bad)
            {
                suppressed++;
                Diagnostics.SinkFlowLog.Post(
                    point.SourceInstanceId, InstanceId, point.TagName,
                    topic: "(suppressed)", value: string.Empty,
                    result: $"SUPPRESSED: quality={point.Quality}"
                            + (point.QualityReason is { Length: > 0 } r ? $" ({r})" : string.Empty));
                continue;
            }
        ...
        // A batch whose points were ALL Bad-quality is fully handled, not
        // failed: nothing was rejected, there was simply nothing worth sending.
        // Without this it would fall through to the failure path below with
        // accepted == 0 and rejected == 0, returning Success=false — which holds
        // the buffer cursor and re-delivers the same undeliverable batch on
        // every dequeue, forever. A dead source would stall its own buffer.
        if (rejected == 0 && suppressed > 0)
        {
            _logger.LogDebug(
                "PerTag: all {Count} point(s) in the batch were Bad quality and were suppressed.",
                suppressed);
            RecordSuccess();
            return new PublishResult
            {
