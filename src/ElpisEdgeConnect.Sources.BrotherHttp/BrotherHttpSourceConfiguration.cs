...
public sealed record BrotherHttpSourceConfiguration : SourceConfiguration
{
    ...
    public static string? TryNormalizeBaseUrl(string? raw)
    {
        ...
        else
        {
            ...
            // "relative", so a plainly malformed address was accepted and only
            ...
            // authority the Host check below rejects — the right answer for
            // something that is not an address at all. Kept identical to
            // MTConnectSourceConfiguration.TryNormalizeAgentBaseUrl: these two
            // normalisers are twins and drifted once already.
