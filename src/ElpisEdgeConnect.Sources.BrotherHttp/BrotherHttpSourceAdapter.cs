...
public sealed class BrotherHttpSourceAdapter : ISourceAdapter, Core.Adapters.Retirement.ISourceRetirement
{
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// Deliberately does NOT probe the CNC. Both outcomes of the old
    /// HTTPD_MCNINFO probe ended at <see cref="AdapterState.Running"/>, and
    /// PollAsync already records exactly the same health success/failure on its
    /// first pass — so the probe decided nothing and only duplicated work.
    ///
    ...
    /// Against a CNC that is not yet wired up — the normal case when an operator
    /// configures ahead of installation — the operator's Save stalled for a full
    /// HTTP timeout per Brother source. Same defect the other adapters were
    /// cleared of (TL-139); Brother's is bounded by one timeout rather than a
    /// retry budget, which made it milder but not harmless.
    /// </remarks>
    public Task StartAsync(CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            "BrotherHttpSourceAdapter '{InstanceId}' started for {BaseUrl}; connecting on first poll.",
