...
internal sealed class MTConnectHttpClient : IMTConnectClient, IDisposable
{
    ...
    public MTConnectHttpClient(MTConnectSourceConfiguration config)
    {
        ...
        // Normalize defensively: FromSourceInstance already does this for
        // config-loaded sources, but a directly-constructed record can still
        // carry a bare host, and BaseAddress below demands an absolute URI.
        var baseUrl = MTConnectSourceConfiguration.TryNormalizeAgentBaseUrl(config.AgentBaseUrl)
            ?? config.AgentBaseUrl;
        if (!baseUrl.EndsWith('/'))
        {
            baseUrl += "/";
