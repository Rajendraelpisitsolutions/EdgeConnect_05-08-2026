...
public sealed class EthernetIpSourceAdapter : ISourceAdapter
{
    ...
    public async Task StartAsync(CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(ex,
                "EthernetIpSourceAdapter '{Id}' initial connect threw — will retry on poll.", _instanceId);
        }

        TransitionState(AdapterState.Running);
        _logger.LogInformation("EthernetIpSourceAdapter '{Id}' started.", _instanceId);
    }
        ...
        EthernetIpTagDefinition tag, List<CanonicalDataPoint> points, CancellationToken ct)
    {
        ...
        points.Add(DecodePoint(tag, elementType, result, txTimestamp));
        return true;
    }
