...
public sealed class EthernetIpSourceAdapter : ISourceAdapter
{
        ...
        EthernetIpTagDefinition tag, List<CanonicalDataPoint> points, CancellationToken ct)
    {
        ...
        if (!result.Success)
        {
            ...
            // Tag-level failures belong in the DEVICE trail too, not only in a
            // Debug-level app log. Previously only the fatal branch above wrote
            // here, so a tag failing on every poll — visibly Bad in the UI —
            // left no trace at all: a support engineer sent to the device log
            // found nothing and would reasonably conclude the tag was healthy.
            // SourceDataLog throttles per key (~30s), and the key is per tag, so
            // a tag failing every 250ms cannot flood the file.
            SourceDataLog.Log("DEVICE", _instanceId, $"tag-read-failed:{tag.Name}",
                "TAG READ FAILED (session healthy)"
                    ...
                    + $" | Tag name: {tag.Name}"
                    + $" | Tag Address: {tag.Address}"
                    + $" | Detail: {result.ErrorCode}: {result.ErrorText}");
            return true;
        }
    ...
    public Task<ValidationResult> ValidateConfigAsync(SourceConfiguration config, CancellationToken ct)
    {
        ...
        // Warnings are collected separately so an address that only needs
        // trimming reports the problem without failing the apply — one dirty
        // address must not take a source's other tags offline.
