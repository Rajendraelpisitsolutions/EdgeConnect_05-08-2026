// ============================================================================
// File: EthernetIpDemoModeOptions.cs
// Purpose: Parser + cache for EDGECONNECT_ETHERNETIP_FAKE_MODE — the SINGLE
//          activation path for EtherNet/IP demo mode. Mirrors
//          S7DemoModeOptions / Focas2DemoModeOptions exactly (ADR-0012 /
//          ADR-0029 pattern): saved configuration cannot enable demo mode;
//          only the process environment variable can, and the value is read
//          once at first access and frozen for the process lifetime (toggling
//          requires a restart).
//
//          ACCEPTED truthy values: true, 1, yes (case-insensitive, trimmed).
//          Anything else (false, 0, no, empty, unset) -> disabled.
// Reference: docs/decisions/0029-s7-demo-mode.md (mirrors ADR-0012)
// ============================================================================

using System;

namespace ElpisEdgeConnect.Sources.EthernetIp;

/// <summary>
/// Activation flag for EtherNet/IP demo mode. Reads
/// <c>EDGECONNECT_ETHERNETIP_FAKE_MODE</c> once and caches the result for the
/// process lifetime. The ONLY supported path for enabling demo mode — saved
/// configuration cannot activate the synthetic backend.
/// </summary>
public static class EthernetIpDemoModeOptions
{
    /// <summary>The single environment variable that activates demo mode.</summary>
    public const string EnvVarName = "EDGECONNECT_ETHERNETIP_FAKE_MODE";

    /// <summary>
    /// Message emitted via stderr / diagnostics at process startup when demo
    /// mode is active. Distinct, grep-friendly phrasing so log monitoring can
    /// pattern-match this specific entry without treating it as a failure.
    /// </summary>
    public const string StartupCriticalMessage =
        "ETHERNETIP FAKE MODE ACTIVE — all Allen-Bradley EtherNet/IP sources use a synthetic PLC. " +
        "Clear EDGECONNECT_ETHERNETIP_FAKE_MODE and restart for production behaviour.";

    private static readonly object _gate = new();
    private static bool? _cached;

    /// <summary>
    /// True when the env var is set to a truthy value at first access.
    /// Frozen for the process lifetime after the first read.
    /// </summary>
    public static bool IsEnabled
    {
        get
        {
            if (_cached is { } cached)
            {
                return cached;
            }
            lock (_gate)
            {
                if (_cached is { } cached2)
                {
                    return cached2;
                }
                _cached = ReadFromEnvironment();
                return _cached.Value;
            }
        }
    }

    private static bool ReadFromEnvironment()
    {
        var raw = Environment.GetEnvironmentVariable(EnvVarName);
        if (string.IsNullOrWhiteSpace(raw))
        {
            return false;
        }
        var trimmed = raw.Trim();
        return string.Equals(trimmed, "true", StringComparison.OrdinalIgnoreCase)
            || trimmed == "1"
            || string.Equals(trimmed, "yes", StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Clear the cached value so the next access re-reads the env var.
    /// Test-cleanup only — no production path calls this.
    /// </summary>
    internal static void Reset()
    {
        lock (_gate)
        {
            _cached = null;
        }
    }
}
