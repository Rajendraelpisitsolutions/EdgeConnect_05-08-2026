...
internal static class EthernetIpErrors
{
    ...
    /// <summary>
    /// The tag definition itself is unusable, so no read request was ever sent.
    /// libplctag raises <c>PLCTAG_ERR_BAD_PARAM</c> locally — the controller was
    /// never contacted. Causes seen in the field: stray whitespace or invalid
    /// characters in the symbolic address, and symbols the controller does not
    /// publish (on Micro800, variables must be declared as Global Variables to
    /// appear in the CIP symbol table).
    /// </summary>
