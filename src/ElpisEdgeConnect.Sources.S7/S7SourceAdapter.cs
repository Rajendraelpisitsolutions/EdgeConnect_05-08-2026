...
//             * StartAsync: transition to Running without touching the
//               wire — the first PollAsync owns the connect.
...
public sealed class S7SourceAdapter : ISourceAdapter, ISourceRetirement
{
    ...
    /// InitializeAsync — and that still reaches Failed with S7.START_FAILED.
    /// </remarks>
    public Task StartAsync(CancellationToken ct)
    {
        if (_config is null || _connection is null)
        {
            ...
            _lastError = MakeError("S7.START_FAILED", ErrorCategory.Internal, fault.Message, retryable: false);
            TransitionState(AdapterState.Failed);
            throw fault;
        }
        ...
        _logger.LogInformation(
            "S7 source {InstanceId} started; connecting on first poll.", _instanceId);
        ...
        SourceDataLog.Session(_instanceId,
            $"S7 source started — endpoint {EndpointLabel}, {_config.TagDefinitions.Count} tag(s), connection deferred to first poll.");
