...
public sealed class LicenseTrialEnforcer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            if (versionRestricted || demoExhausted)
            {
                if (!dataHalted)
                {
    ...
    private bool CheckClockRollback()
    {
        ...
        var realElapsed = _lastMonotonicMs < 0
            ? TimeSpan.Zero
            : TimeSpan.FromMilliseconds(Math.Max(0, nowMonotonic - _lastMonotonicMs));
        ...
        if (!rolledBack)
        {
            // Clock is sane — move the high-water mark forward (capped to the real
            // elapsed time so a forward jump can't poison it) and clear any sticky
            // before-issue / tamper state.
            _minValidUtc = null;
            _clockAnchors.Write(ClockRollbackPolicy.Advance(now, anchor, realElapsed, _clockTolerance));
    ...
    /// </list>
    /// </summary>
    private async Task SyncLicenseFileAsync(CancellationToken cancellationToken)
    {
...
#pragma warning disable CA1031 // a bad license file must never crash the host
        try
        {
            ...
            var unchanged = _license.Current is not null
