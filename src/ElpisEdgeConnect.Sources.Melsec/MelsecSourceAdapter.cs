...
//            * StartAsync: transition to Running without touching the wire —
//              the first PollAsync owns the connect.
...
public sealed class MelsecSourceAdapter : ISourceAdapter, ISourceRetirement, IMelsecDiagnosticsProvider
{
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// Deliberately does NOT dial the PLC. PollAsync calls
    /// <c>EnsureConnectedAsync</c> itself, so the eager connect here bought
    /// nothing — but it was awaited by <c>SourceSupervisor.AddAsync</c> and
    /// therefore by the hot-reload coordinator, so applying a config against a
    /// PLC that is not wired up yet (the normal case when an operator
    /// configures ahead of installation) burned the connect budget and stalled
    /// the operator's Save until the apply endpoint's ReloadOutcomeWaitMs cap
    /// expired. Connecting is now the poll loop's job alone, which is where the
    /// backoff, the circuit breaker and the operator-facing fault reporting
    /// already live. Matches FOCAS2 / Modbus TCP / EtherNet/IP (commit 6c1d984).
    ///
    /// A device that is silent is NOT a start failure: it left this method via
    /// <c>EnsureConnectedAsync</c> returning false, never via a throw. The only
    /// genuine start fault is an internal one — being started before
    /// InitializeAsync — and that still reaches Failed with MELSEC.START_FAILED.
    /// </remarks>
    public Task StartAsync(CancellationToken ct)
    {
        if (_config is null || _connection is null)
        {
            // Internal fault (lifecycle misuse), not an unreachable device — the
            // adapter genuinely cannot start, so it is Failed with a diagnosable code.
            var fault = new InvalidOperationException("MelsecSourceAdapter.StartAsync called before InitializeAsync.");
            _lastError = MakeError("MELSEC.START_FAILED", ErrorCategory.Internal, fault.Message, retryable: false);
            TransitionState(AdapterState.Failed);
            throw fault;
        }
        ...
        _logger.LogInformation("MELSEC source {InstanceId} started; connecting on first poll.", _instanceId);

        // "connection deferred", never "connected=false": no connection has been
        // attempted at this point, and an operator reading connected=false in the
        // session log would take it as a device that answered and refused.
        SourceDataLog.Session(_instanceId,
            $"MELSEC source started — endpoint {EndpointLabel}, {_config.TagDefinitions.Count} tag(s), connection deferred to first poll.");
