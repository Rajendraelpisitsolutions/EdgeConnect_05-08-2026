...
internal sealed class ModbusConnectionManager : IAsyncDisposable
{
    ...
    public async Task<bool> EnsureConnectedAsync(CancellationToken ct)
    {
        ...
        finally
        {
            if (!opened)
            {
                // Every abandoned connect attempt must close its transport before
                // we enter backoff — otherwise each retry cycle opens a new socket
                // and orphans the previous one, and the gateway itself exhausts the
                // device's connection limit. The client-level cleanup already does
                // this; repeating it here is deliberate defence in depth, because
                // this is the one place that KNOWS the attempt was abandoned
                // (including cancellation and any non-mapped exception).
                // Disconnect() is idempotent and swallows its own failures, so it
                // cannot turn a connect failure into an adapter-level fault
                // (locked decision #10: per-adapter isolation).
                Disconnect();
            }
        }
    }
    ...
    /// <summary>
    /// Disconnect the underlying client. Idempotent, lock-free, and
    /// <b>unconditional</b> — it is never gated on a "connected" flag, because
    /// the paths that most need the close (faulted read, failed connect) are
    /// exactly the ones where that flag reads false.
    /// </summary>
