...
public sealed class ModbusTcpSourceAdapter : ISourceAdapter, ISourceRetirement
{
    ...
    // Operator-facing labels for the per-source diagnostic log — reflect the
    // actual Modbus transport (TCP / RTU-over-TCP / serial RTU) rather than the
    // shared adapter's project name.
    private string SourceTypeLabel => _config?.Encapsulation switch
    {
        ModbusEncapsulation.SerialRtu => "Modbus RTU (serial)",
        ModbusEncapsulation.RtuOverTcp => "Modbus RTU (over TCP)",
        _ => "Modbus TCP",
    };

    private string EndpointLabel => _config?.Encapsulation == ModbusEncapsulation.SerialRtu
        ? (_config?.SerialPort ?? "serial")
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// No initial connect here — PollAsync calls EnsureConnectedAsync itself.
    /// The eager attempt was best-effort and its result discarded, but it was
    /// awaited by SourceSupervisor.AddAsync and therefore by the hot-reload
    /// coordinator, so applying a config against an unreachable device blocked
    /// the operator's Save for the connect budget. Connecting belongs to the
    /// poll loop, which owns the backoff and circuit breaker.
    /// </remarks>
    public Task StartAsync(CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            "ModbusTcpSourceAdapter '{Id}' started; connecting on first poll.", _instanceId);
        ...
        DataIssueLog.Session(_instanceId,
            $"source STARTED — endpoint={EndpointLabel} ({_config?.Encapsulation}); "
    ...
    public Task<AdapterHealth> CheckHealthAsync(CancellationToken ct)
    {
        ...
        {
            metrics["endpoint"] = EndpointLabel;
    ...
    public async Task<IReadOnlyList<CanonicalDataPoint>> PollAsync(CancellationToken ct)
    {
        ...
        try
        {
            if (!await _connectionManager.EnsureConnectedAsync(ct).ConfigureAwait(false))
            {
                ...
                RecordFailure(MakeError(
                    ...
                    $"{SourceTypeLabel} device {EndpointLabel} is not reachable — no connection "
                ...
                DataIssueLog.Log("DEVICE", _instanceId,
                    ...
                    "CONNECT FAILED"
                        ...
                        + $" | Status: Disconnected"
                        + $" | Consecutive failures: {_connectionManager.ConsecutiveFailures}"
                        + $" | Circuit breaker: {_connectionManager.BreakerState}"
                        + $" | Connect timeout ms: {_config?.ConnectTimeoutMs}"
                        + $" | Last connected at: {(_connectionManager.LastSuccessAt?.ToString("O") ?? "never")}"
                        + " | Probable cause: device not reachable (wrong IP/subnet, TCP port closed/filtered, "
                        + "or the endpoint is not a Modbus server) — verify with ping + Test-NetConnection <host> -Port <port>"
        ...
        catch (Exception ex)
        {
            ...
            DataIssueLog.Log("CODE", _instanceId, "poll-exception",
                $"POLL ERROR — endpoint={EndpointLabel} ({_config?.Encapsulation}); "
        ...
        ScanGroup group, ScanBlock block, List<CanonicalDataPoint> points, CancellationToken ct)
    {
        ...
        if (!result.IsSuccess)
        {
            ...
            DataIssueLog.Log("DEVICE", _instanceId,
                ...
                $"READ FAILED — endpoint={EndpointLabel} ({_config?.Encapsulation}); "
        ...
        foreach (var entry in block.Entries)
        {
            try
            {
                ...
                DataIssueLog.Log("DATA", _instanceId,
                    ...
                    "READ OK"
                        ...
                        + $" | Endpoint: {EndpointLabel}"
                        + $" | Status: Connected"
                        + $" | Tag name: {point.TagName}"
                        + $" | Tag Address: {group.RegisterClass}/{entry.Tag.Address}"
                        + $" | Unit ID: {entry.Tag.UnitId}"
                        + $" | Datatype: {entry.Tag.Datatype ?? "uint16"}"
                        + $" | Byte order: {(entry.Tag.ByteOrder?.ToString() ?? "default")}"
            ...
            catch (Exception ex)
            {
                ...
                DataIssueLog.Log("CODE", _instanceId,
                    ...
                    $"DECODE FAILED — endpoint={EndpointLabel}; "
