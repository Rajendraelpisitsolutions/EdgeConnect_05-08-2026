      ...
      IDiagnosticsService
{
    ...
    public void OnSinkRecovered(SinkRecoveredEvent evt)
    {
        ...
        lock (_gate)
        {
            ...
            // Recovery: clear the stale error, mirroring the source-side
            // clear in RecordSourceState. Recovery is the END of the
            // degraded -> draining -> recovered cycle (SinkPublisher only
            // emits it once the drain completes), so the sink is both
            // reconnected AND caught up — nothing is left for the operator
            // to act on. Without this, RouteCard renders its fault panel off
            // LastErrorAtUtc, not IsDegraded, so a sink that dropped for
            // milliseconds showed a red fault for the rest of the uptime.
            // The error is deliberately RETAINED while still degraded or
            // draining; the history stays in the Events ring below.
            sink.LastError = null;
            sink.LastErrorAtUtc = null;
            sink.Events.Add(new SinkEventEntry
            {
        ...
        AdapterError? lastError)
    {
        ...
        lock (_gate)
        {
            ...
            // A sink's LastError is shared with the publish-level dimension
            // (OnSinkDegraded / OnSinkRecovered), so clearing it here needs more
            // care than the source path: the adapter can be Running (broker
            // connected) while DELIVERY still fails, and blanking the error then
            // would hide a live fault.
            //
            // But refusing to clear it at all left the mirror of the bug fixed in
            // OnSinkRecovered. An ADAPTER-level fault — a broker that refused the
            // first connect, say — is recorded here and cleared nowhere:
            // OnSinkRecovered only fires at the end of a
            // degraded -> draining -> recovered PUBLISH cycle, which never
            // happens if the sink was never publish-degraded. The error then
            // outlives the outage for the rest of the gateway's uptime, and
            // RouteHealth folds a live sink error into the route verdict, so the
            // route reads broken while it publishes normally.
            //
            // Clear only when BOTH dimensions agree there is nothing outstanding:
            // the adapter reports Running with no error, and no publish-level
            // degradation or drain is in flight. A delivery fault still owns the
            // error until OnSinkRecovered releases it.
            else if (state == AdapterState.Running && !sink.IsDegraded && !sink.IsDraining)
            {
                sink.LastError = null;
                sink.LastErrorAtUtc = null;
            }
