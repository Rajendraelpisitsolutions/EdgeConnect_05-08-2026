...
internal sealed class RouteWorker
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // Reconcile the buffer's sink cursors with THIS route's configured sinks BEFORE
        // the intake pump starts, so that any points written to the buffer are observed
        // by every sink. The dispatcher is pre-registered by the routing engine so a wake
        // issued during startup is not lost.
        //
        // Reconcile — not a register loop. A route buffer file is keyed by RouteId, so a
        // route whose sink was deleted and replaced keeps the OLD sink's cursor in its
        // cursors table. Nothing ever deleted it (DeregisterSinkAsync has no production
        // caller), it was resurrected on every open, and it pinned tail_sequence forever:
        // unbounded disk growth plus a phantom backlog on a route that is delivering
        // perfectly. Registering is therefore only half the job; the stale cursors have to
        // go at the same time.
        var activeSinkIds = new List<string>(def.Sinks.Count + 1);
        foreach (var sink in def.Sinks)
        {
            activeSinkIds.Add(sink.InstanceId);
        }

        // A replay route's sink is driven by ReplayRouteDriver rather than appearing in
        // def.Sinks in every configuration shape. Include it explicitly so reconciliation
        // can never see it as unconfigured. (The store force-adds it as well — belt and
        // braces, because pruning it would strip generation fencing.)
        // Deduped: for a replay route the id normally IS the single configured sink's
        // InstanceId, and the default (register-only) reconcile body would otherwise call
        // RegisterSinkAsync for it twice.
        if (_route.Replay is { } replayContext && !activeSinkIds.Contains(replayContext.SinkId))
        {
            activeSinkIds.Add(replayContext.SinkId);
        }

        // A route with NO configured sinks is a configuration error, not evidence that
        // every cursor is stale — reconciling against an empty set would prune the whole
        // cursor table and discard the buffered backlog. Skip rather than destroy.
        if (activeSinkIds.Count > 0)
        {
            var reconciliation = await _route.Buffer
                .ReconcileSinksAsync(activeSinkIds, ct)
                .ConfigureAwait(false);

            ReportPrunedSinkCursors(def.RouteId, reconciliation);
        }
    ...
    /// <summary>
    /// Surface every pruned sink cursor at WARNING severity. Pruning DISCARDS that sink's
    /// undelivered backlog and drops the route's reported depth, so it must never happen
    /// silently — an operator who sees depth collapse has to be able to find out why.
    /// </summary>
    /// <remarks>
    /// Core has no ILogger seam by design (it stays host- and protocol-agnostic), so this
    /// goes to stderr with an explicit WARN marker, the same last-resort visibility channel
    /// RoutingEngine uses for unhandled worker faults. Writing is best-effort: a failing
    /// console must never take down a route that is otherwise healthy.
    /// </remarks>
    private static void ReportPrunedSinkCursors(string routeId, SinkReconciliationResult reconciliation)
    {
        if (reconciliation.Pruned.Count == 0)
        {
            return;
        }

        foreach (var pruned in reconciliation.Pruned)
        {
            try
            {
                Console.Error.WriteLine(
                    $"[WARN] [route {routeId}] pruned stale buffer cursor for sink '{pruned.SinkId}': " +
                    $"it is no longer configured on this route. It was pinning sequence {pruned.PinnedSequence} " +
                    $"and had {pruned.UndeliveredPoints} undelivered point(s), which are now discarded for that " +
                    $"sink and reclaimable. Buffer depth for this route will drop accordingly.");
            }
            catch { /* diagnostics never break route startup */ }
        }
    }
