...
public sealed class InMemoryBuffer : IMessageBuffer
{
    ...
    /// <inheritdoc/>
    public ValueTask<SinkReconciliationResult> ReconcileSinksAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Same semantics as the durable store, for a route re-pointed at a new sink
        // within a single process: register what is missing, prune what is no longer
        // configured, and release what only the pruned cursors were pinning. There is no
        // transaction here because the single lock IS the atomic boundary — the whole
        // set change and the tail recompute happen without any other thread observing
        // an intermediate cursor set.
        TaskCompletionSource<bool>? toRelease = null;
        ...
        lock (_lock)
        {
            ...
            // Snapshot before mutating — Deregister would otherwise change the set we
            // are walking.
            ...
            foreach (var (sinkId, cursor) in _cursors.Snapshot())
            {
                ...
                {
                    // CONFIGURED — never pruned, however far it lags.
                    continue;
                }
            ...
            {
                return new ValueTask<SinkReconciliationResult>(SinkReconciliationResult.Empty);
            }
        ...
        toRelease?.TrySetResult(true);
        return new ValueTask<SinkReconciliationResult>(result);
    }
