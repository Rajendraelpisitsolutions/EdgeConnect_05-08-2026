...
public sealed class SinkCursorTracker
{
    ...
    /// <summary>
    /// Copy the whole (sinkId, cursor) set out under the internal lock.
    /// </summary>
    /// <remarks>
    /// Reconciliation needs to decide which sinks to remove and then remove
    /// them. Iterating <see cref="Count"/>-style accessors while calling
    /// <see cref="Deregister"/> would mutate the backing dictionary mid-walk;
    /// taking a detached snapshot first makes that safe without holding the
    /// lock across the mutation.
    /// </remarks>
    /// <returns>A detached copy of the current cursor set.</returns>
    public IReadOnlyList<(string SinkId, long Cursor)> Snapshot()
    {
        lock (_lock)
        {
            var snapshot = new (string, long)[_cursors.Count];
            var i = 0;
