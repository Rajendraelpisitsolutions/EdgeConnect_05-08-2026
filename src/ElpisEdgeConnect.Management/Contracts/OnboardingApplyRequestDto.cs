...
public sealed record OnboardingApplyRequestDto
{
    ...
    /// <summary>
    /// True when the operator already pressed Save on the source step, so the
    /// source exists on the gateway (disabled) and this apply is delivering its
    /// final form. The merger then REPLACES it instead of rejecting the id as a
    /// duplicate. False for the ordinary flow, where the source is new here.
    /// </summary>
    public bool SourceAlreadyCreated { get; init; }

    /// <summary>Destination twin of <see cref="SourceAlreadyCreated"/>.</summary>
    public bool SinkAlreadyCreated { get; init; }
