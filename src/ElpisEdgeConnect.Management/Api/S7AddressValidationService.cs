...
public sealed class S7AddressValidationService
{
    ...
    private static (string Code, string Message) FriendlyError(string raw)
    {
        ...
        // Labelled examples, not a bare list. An operator who typed "1" has
        // given a tag NUMBER; five unexplained tokens tell them they are wrong
        // without telling them what the field wants. Naming what each example
        // IS — data block, memory, input, output — is what lets someone map it
        // onto the address they already have written down for that PLC.
        return (Code,
            $"'{raw}' is not an S7 address. An address says where the value lives in the PLC: "
                + "a data block like DB1.DBW0, a memory bit like M10.2, an input like IW4, "
                + "or an output like Q0.1.");
    }
}
