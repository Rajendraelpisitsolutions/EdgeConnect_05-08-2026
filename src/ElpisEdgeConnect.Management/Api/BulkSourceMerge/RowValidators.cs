...
//            * MtConnect URL  — v3.1 section 9 (must be absolute http/https Uri)
...
public static class RowValidators
{
    ...
    /// <summary>
    /// Validate an MTConnect baseUrl cell per v3.1 §9: must parse as an
    /// absolute Uri with scheme <c>http</c> or <c>https</c>. Other schemes
    /// (<c>file://</c>, <c>javascript:</c>, <c>ftp://</c>, etc.) reject.
    /// </summary>
    public static BulkSourceMergeFinding? ValidateMtConnectBaseUrl(string baseUrl, int lineNumber)
    {
        ...
        if (!Uri.TryCreate(baseUrl, UriKind.Absolute, out var uri))
        {
            return new BulkSourceMergeFinding
            {
                Code = BulkSourceMergeErrorCode.MtConnectBaseUrlInvalid,
                Severity = BulkSourceMergeSeverity.Error,
                Message = $"baseUrl '{Truncate(baseUrl)}' is not an absolute URI.",
                CsvRow = lineNumber,
                Subject = baseUrl,
            };
        }
            ...
            && !string.Equals(uri.Scheme, Uri.UriSchemeHttps, StringComparison.Ordinal))
        {
            return new BulkSourceMergeFinding
            {
                Code = BulkSourceMergeErrorCode.MtConnectBaseUrlInvalid,
                Severity = BulkSourceMergeSeverity.Error,
                Message = $"baseUrl scheme '{uri.Scheme}' is not supported. Use http or https.",
                CsvRow = lineNumber,
                Subject = baseUrl,
            };
        }
