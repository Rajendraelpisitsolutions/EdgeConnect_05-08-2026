...
public sealed class MTConnectBrowseService
{
    ...
    public async Task<MTConnectBrowseResultDto> BrowseAsync(MTConnectBrowseRequest request, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.AgentBaseUrl) ||
            !Uri.TryCreate(request.AgentBaseUrl, UriKind.Absolute, out _))
        {
            return Fail(MTConnectBrowseStatus.Unreachable, sw, "Agent base URL is missing or not an absolute URL.");
        }

        var fetch = await _fetcher.FetchProbeAsync(request.AgentBaseUrl, ProbeBudget, ct).ConfigureAwait(false);
