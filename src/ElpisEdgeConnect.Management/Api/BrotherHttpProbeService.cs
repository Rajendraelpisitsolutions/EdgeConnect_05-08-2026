...
public sealed class BrotherHttpProbeService
{
        ...
        if (normalisedBaseUrl is null)
        {
            return new BrotherHttpProbeOutcome(
                ...
                Result: new BrotherHttpProbeResultDto
                {
                    ...
                    ErrorMessage = BrotherHttpSourceConfiguration.InvalidBaseUrlMessage(request.BaseUrl),
                });
        }
    ...
    /// trailing-slash stripped, scheme preserved — plus the implied
    /// <c>http://</c> for a bare host, IP, or <c>host:port</c>. Returns null
    /// when the input still isn't a usable absolute http(s) URL.
    /// </summary>
    /// <remarks>
    /// Delegates to <see cref="BrotherHttpSourceConfiguration.TryNormalizeBaseUrl"/>
    /// deliberately: the probe, the adapter, and a hand-written or bulk-imported
    /// gateway.json must all agree on what an operator-typed address means, so
    /// the rule has exactly one implementation and it lives with the protocol
    /// module, not in the Studio.
    /// </remarks>
    internal static string? NormaliseBaseUrl(string? raw) =>
        BrotherHttpSourceConfiguration.TryNormalizeBaseUrl(raw);
...
{
    /// <summary>
    /// Brother CNC address, e.g. <c>"http://192.168.2.110"</c> or the bare
    /// <c>"192.168.2.110"</c> — <c>http://</c> is implied when absent.
    /// </summary>
