...
public static class SourceProtocolPickerModel
{
    ...
    /// <summary>
    /// Resolve the catalogue for a given license. Under a restricted edition only
    /// the offered source tiles are returned — all others are hidden entirely (not
    /// merely locked). Which tiles are offered is decided per protocol by
    /// <see cref="LicenseEditionCatalog.IsSourceModuleOffered"/> (mirrors the
    /// DestinationProtocolPickerModel pattern), so Professional correctly hides
    /// OPC UA Client and CNC Pro hides everything but its three CNC sources —
    /// this does not rely on <see cref="LicenseEditionCatalog.RestrictsSources"/>,
    /// which stays Starter/CNC-Pro-shaped for the License Generator's benefit.
    /// Of the remaining tiles, any whose
    ...
    {
        // Dev / no license loaded and no edition restriction: catalogue untouched.
        if (isModuleEnabled is null && edition == LicenseEdition.Unknown)
        {
            return Tiles;
        }
        ...
        {
            // Edition offering gate: a tile is only offered when its license
            // module is offered under the edition (Pending/no-module tiles are
            // never offered under a named edition).
            var offered = tile.RequiredLicenseModule is { } offeredModule
                ? LicenseEditionCatalog.IsSourceModuleOffered(edition, offeredModule)
            ...
            // Starter surfaces exactly the Modbus TCP tile; the Modbus RTU tile
            // shares the source-modbus-tcp module but is a distinct variant Starter
            // does not offer.
            if (edition == LicenseEdition.Starter && tile.Key == "modbus-rtu")
            {
                continue;
            }
