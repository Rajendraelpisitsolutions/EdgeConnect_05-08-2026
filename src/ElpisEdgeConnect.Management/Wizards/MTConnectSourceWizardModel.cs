...
public sealed class MTConnectSourceWizardModel
{
    ...
    public SourceInstanceConfig BuildSourceInstance()
    {
        ...
        {
            // Stored canonical, so the value the operator typed as a bare host
            // reads back the same way it will be requested. An unusable address
            // is written through unchanged for config validation to reject with
            // a message, rather than being silently dropped here.
            [MTConnectConnectionKeys.AgentBaseUrl] =
                MTConnectSourceConfiguration.TryNormalizeAgentBaseUrl(AgentBaseUrl) ?? AgentBaseUrl.Trim(),
