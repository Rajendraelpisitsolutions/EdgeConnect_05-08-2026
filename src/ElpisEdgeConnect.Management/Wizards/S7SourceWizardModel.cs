...
public sealed class S7SourceWizardModel
{
    ...
    private static bool TryParseAddress(string address, out S7Address parsed, out ValidationIssue? error)
    {
        ...
        {
            // Kept word-for-word in step with S7AddressValidationService — the
            // operator can reach either path from the same field, and two
            // different sentences for one mistake reads as two different faults.
            error = Issue(
                $"'{address}' is not an S7 address. An address says where the value lives in the PLC: "
