...
public static class ManagementHostingExtensions
{
        ...
        ILicenseManager? license = null)
    {
        ...
        // Notifications sit centred at the top rather than in the right-hand
        // corner. On a wide plant monitor the corner is the furthest point from
        // where the operator is looking — they have just pressed a button in
        // the middle of the screen — so a "Saved" confirmation there is easy to
        // miss entirely. Top-centre keeps it in the line of sight without
        // covering the controls the operator is about to use next, which a
        // dead-centre overlay would.
        builder.Services.AddMudServices(config =>
        {
            // Fully qualified: only MudBlazor.Services is imported in this file,
            // and a blanket `using MudBlazor` here would collide with several
            // local type names.
            config.SnackbarConfiguration.PositionClass =
                MudBlazor.Defaults.Classes.Position.TopCenter;
