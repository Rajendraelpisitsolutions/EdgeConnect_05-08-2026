# K3 Slice 6 pass 2 — Exact Source Diff r3 (disposal linearization)
...
```diff
commit ebd7278a4a6b460182245bb73573c66d2a7ade90
Author: Sudhakar <sudhakar@elpisitsolutions.com>
Date:   Thu Jul 23 17:15:21 2026 +0530

    fix(sparkplug): K3 slice 6 pass 2 review r3 - disposal linearization
    
    Make disposal ownership, fail-closed guarding, and shared completion one
    synchronized decision: _disposeTask (installed via Interlocked.CompareExchange
    before the gate) is the single linearization marker every gated surface reads
    through DisposalWon. This closes the window where _disposeTask was installed
    but the old _disposed flag was still 0 — a Begin/Initialize that acquired the
    gate after ownership was won could previously slip through. Add the missing
    guards on InitializeAsync and EndSessionAsync (an End that loses to disposal is
    a no-op: no NDEATH, no clean DISCONNECT — disposal owns the abort-retirement).
    
    Tests: extend the after-dispose theory to initialize/start/cutover; queued-
    behind-disposal fails closed with no resurrection; concurrent Dispose caller
    awaits the shared retirement (never completes before caller A); End losing to
    disposal emits no death/disconnect. Focused recovery evidence: identity-store
    failure in PrepareBirth fails once with no backoff and no new transport;
    generation exhaustion is fatal with no backoff and no bdSeq reserved; End
    during backoff aborts the recovery cleanly and leaves ready-no-session.
    
    538 tests pass, broker-free and deterministic. No Core change.
...
@@ -42,1628 +42,1639 @@ namespace ElpisEdgeConnect.Sinks.SparkplugB.Session;
 /// <summary>
...
@@ -36,1170 +36,1366 @@ namespace ElpisEdgeConnect.Sinks.SparkplugB.Tests.Session;
