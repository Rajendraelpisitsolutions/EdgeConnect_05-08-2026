# K3 Slice 6 pass 2 — Exact Source Diff r5 (early atomic recovery claim)
...
Full `git show -W` (2 files, 0 elision) for line-level sign-off.

```diff
commit 3513cdc4040420bf43fb6bc087e01d8e15c2b3c2
Author: Sudhakar <sudhakar@elpisitsolutions.com>
Date:   Fri Jul 24 10:08:01 2026 +0530

    fix(sparkplug): K3 slice 6 pass 2 review r5 - claim recovery ownership before the first await
    
    Close the pre-token window in SuspectRebirthAsync. Ownership was published only
    after the initial suspect-transport retirement and non-retryable birth prep, so
    disposal could win while _activeRecoveryToken was still null, after which the
    recovering call could resume, install a fresh token, reserve a bdSeq, issue a
    generation and open a transport.
    
    Claim the token ATOMICALLY (Interlocked.CompareExchange null -> token) as the
    very first statement, before any await — the CAS also enforces the single-
    recovery invariant (a second recovery fails the CAS and is rejected nonfatally
    with OperationCanceledException). Wrap the whole body so the finally CAS-clears
    the token only if still ours. Introduce ValidateRecoveryOwnership(token), which
    throws OperationCanceledException on DisposalWon || token-mismatch, and call it
    before the first await, after the initial retirement, before each connection
    attempt, and inside BackoffWithGateReleasedAsync (now routed through the same
    helper). No transport/generation/bdSeq work proceeds once ownership is lost, in
    any interleaving.
    
    Test: Dispose_DuringInitialSuspectRetirement_SupersedesRecovery_BeforePrepareOrAttempt
    blocks recovery inside the initial previous-transport DisposeAsync, lets disposal
    win, releases the retirement, and asserts recovery aborts before PrepareBirth or
    any attempt (no new transport/generation/bdSeq, no alias resolve, no candidate
    promoted, OperationCanceledException, terminal Stopped/Stopped).
    
    540 tests pass, broker-free and deterministic. No Core change.
...
@@ -42,1650 +42,1666 @@ namespace ElpisEdgeConnect.Sinks.SparkplugB.Session;
 /// <summary>
...
-        // Abort if disposal has won OR another lifecycle call invalidated the token. Checking DisposalWon
-        // explicitly (not only the token) makes the recovery loop honor the SAME disposal linearization point
-        // as the public surfaces, so it can never enter another AttemptConnectionAsync after disposal (r3).
-        if (DisposalWon || !ReferenceEquals(_activeRecoveryToken, token))
-        {
-            throw new OperationCanceledException(
-                "the transport recovery was superseded by disposal or another lifecycle call.");
...
@@ -36,1413 +36,1461 @@ namespace ElpisEdgeConnect.Sinks.SparkplugB.Tests.Session;
...
+        public TaskCompletionSource? DisposeEntered { get; set; } // signalled when a gated retirement starts blocking
         ...
         {
-            if (DisposeGate is { } gate) { await gate; }
