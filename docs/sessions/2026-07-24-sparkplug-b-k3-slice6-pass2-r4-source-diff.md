# K3 Slice 6 pass 2 — Exact Source Diff r4 (recovery-invalidation linearization)
...
```diff
commit 13e597332ae686fb6968f7b8384c15bdb538f6c1
Author: Sudhakar <sudhakar@elpisitsolutions.com>
Date:   Fri Jul 24 09:28:46 2026 +0530

    fix(sparkplug): K3 slice 6 pass 2 review r4 - linearize recovery invalidation with disposal
    
    Close the marker-vs-token interval: DisposeAsync nulls _activeRecoveryToken
    BEFORE the ownership CAS, so invalidating the in-flight recovery and winning
    disposal are one ordering decision. Previously the token was nulled only later
    in DisposeCoreAsync, leaving a window where a recovery parked in gate-released
    backoff could reacquire the gate, observe a still-current token, and begin
    another full attempt (reserving a bdSeq, issuing a generation, creating a
    transport) before disposal invalidated it. Every concurrent Dispose caller may
    null the token idempotently; any Dispose supersedes recovery.
    
    Belt-and-suspenders: BackoffWithGateReleasedAsync now aborts on DisposalWon
    OR a token mismatch after reacquiring the gate, so the recovery loop honors the
    same disposal linearization point as the public surfaces and never enters
    another AttemptConnectionAsync after disposal has won.
    
    Test: Dispose_DuringRecoveryBackoff_SupersedesRecovery_NoNewAttempt drives the
    exact ordering (recovery in controlled backoff → disposal wins → retirement
    held on a transport-dispose barrier → backoff released → recovery aborts) and
    asserts no next transport/generation/bdSeq, OperationCanceledException, terminal
    Stopped/Stopped, and no candidate authority promoted.
    
    539 tests pass, broker-free and deterministic. No Core change.
...
@@ -42,1639 +42,1650 @@ namespace ElpisEdgeConnect.Sinks.SparkplugB.Session;
 /// <summary>
...
@@ -36,1366 +36,1413 @@ namespace ElpisEdgeConnect.Sinks.SparkplugB.Tests.Session;
