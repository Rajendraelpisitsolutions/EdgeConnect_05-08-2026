"""
File:     docs/marketing/assets/build-pitch-deck-v7.py
Purpose:  Generate the CANONICAL executive pitch deck (pitch-deck-v7.pptx) for
          the Elpis Industrial Intelligence Platform. v7 is the hardware-story
          deck: the spine is the five-capability ecosystem — a five-pillar
          overview (slide 4), a dedicated acquisition-hardware slide (slide 5),
          and a software-depth slide (slide 6, EdgeConnect + EREMOS V2 as the
          bookends of the stack); architecture / why / editions reframed to the
          ecosystem. (Promoted "v7b" lineage; the shorter 13-slide "evolve"
          variant is archived at build-pitch-deck-v7a.py — fallback only.)
...
Output:   docs/marketing/assets/pitch-deck-v7.pptx
...
v6 -> v7b (Track A hardware-story expansion, VARIANT B — "ecosystem rebuild"):
  Restructures the spine around the locked 5-capability ecosystem story
  (homepage v2 / datasheet v5):
    - Title slide -> "Industrial Intelligence Ecosystem" + ecosystem subtitle.
    - Slide 4 "Five capabilities. One ecosystem." (the 5-pillar strip).
    - NEW slide 5 "Acquisition hardware — built by Elpis" (Edge Gateway, mDAQ,
      mTracker, VAS, E-IDOS) + LOCKED anonymized proof anchors.
    - NEW slide 6 "The software that runs it" (EdgeConnect + EREMOS V2 depth —
      the two products framed as the bookends of the five capabilities).
    - Architecture caption, Why-Elpis, and editions reframed to the ecosystem
      (software editions; hardware scoped per BOM).
  Deck grows 12 -> 14 slides. Protocol reality stays correct from v6 (S7 +
  OPC UA Client shipped; MT-LINKi roadmap). Hardware specs kept high-level /
  qualitative (no fabricated numbers); E-IDOS framed standalone-today.

v7b -> v7 (canonical, 2026-06-05): user picked the v7b lineage as master and
  archived v7a as a shorter 13-slide fallback. ChatGPT claim-hardening applied
  before lock:
    - Cover title restored to the canonical "Industrial Intelligence Platform"
      (keeping v7b's stronger ecosystem subtitle).
    - P0: slide 6 "every controller" overclaim -> "supported CNC, PLC, and
      instrumentation sources" (don't overrun the protocol slide's discipline).
    - P0: AI line on Why-Elpis reframed to "AI architecture: proposals require
      human confirmation" so it can't read as shipped-today vs the roadmap slide.
    - P1: outcome "Pass your audit" -> "Make audit review defensible".
    - P1: editions roadmap presenter note reframed (don't blur shipped vs planned).

v1-v6 retained as historical reference; v7a archived as the 13-slide fallback.

Run:      python docs/marketing/assets/build-pitch-deck-v7.py
"""
...
OUT_PPTX = ASSETS_DIR / "pitch-deck-v7.pptx"
...
def content_chrome(slide, slide_num, total=14):
...
# Subtitle
add_text(s, "From shop floor signal to enterprise decision — connectivity, acquisition, "
            "condition monitoring, and operational intelligence in one stack.",
         Inches(0.95), Inches(4.3), Inches(11.3), Inches(1.2),
         size=21, color=TEXT_MUTED, italic=True, line_spacing=1.3)
...
add_text(s, "Five capabilities. One ecosystem.",
         Inches(0.7), Inches(0.95), Inches(11.9), Inches(0.9),
         size=34, bold=True, color=TEXT_HEADING, line_spacing=1.1)

# v7a: five-capability strip (the locked ecosystem story; mirrors homepage section 2).
# Each pillar = one card: eyebrow + capability + product footer. Software pillars
# (1 Connectivity & Edge, 5 Operational Intelligence) get a stronger teal cue.
pillars = [
    ("PILLAR 1", "Connectivity & Edge",
     "Brownfield-ready connectivity to the controllers you already run.",
     "EDGECONNECT · EDGE GATEWAY"),
    ("PILLAR 2", "Data Acquisition",
     "Sensor-direct acquisition where controllers won't share data.",
     "MDAQ"),
    ("PILLAR 3", "Asset Intelligence",
     "Utilization telemetry — even on unconnected legacy machines.",
     "MTRACKER"),
    ("PILLAR 4", "Condition Monitoring",
     "Vibration and oil-health intelligence — sensor-agnostic.",
     "VAS · E-IDOS"),
    ("PILLAR 5", "Operational Intelligence",
     "Multi-tenant OEE, alarms, and reports across plants.",
     "EREMOS V2"),
]
card_w = Inches(2.18)
card_h = Inches(3.35)
strip_y = Inches(2.35)
start_x = 0.7
for i, (eyebrow, title, body, footer) in enumerate(pillars):
    x = Inches(start_x + i * (2.18 + 0.26))
    is_software = i in (0, 4)
    add_rect(s, x, strip_y, card_w, card_h, fill_rgb=SURFACE_HERO,
             line_rgb=BRAND_TEAL if is_software else BORDER_SUBTLE,
             line_w=Pt(1.5) if is_software else Pt(1.0))
    add_rect(s, x, strip_y, card_w, Inches(0.07), fill_rgb=BRAND_TEAL)
    add_text(s, eyebrow, x + Inches(0.22), strip_y + Inches(0.26),
             card_w - Inches(0.4), Inches(0.3),
             size=9, color=BRAND_TEAL, bold=True, letter_spacing=2.0)
    add_text(s, title, x + Inches(0.22), strip_y + Inches(0.6),
             card_w - Inches(0.4), Inches(0.9),
             size=16, bold=True, color=TEXT_HEADING, line_spacing=1.05)
    add_text(s, body, x + Inches(0.22), strip_y + Inches(1.6),
             card_w - Inches(0.44), Inches(1.2),
             size=11, color=TEXT_MUTED, line_spacing=1.25)
    add_text(s, footer, x + Inches(0.22), strip_y + card_h - Inches(0.55),
             card_w - Inches(0.4), Inches(0.45),
             size=9, color=TEXT_CAPTION, bold=True, line_spacing=1.1)

# Tagline at bottom centered
add_text(s, "Every layer of the industrial data path — collected, normalized, and understood. "
            "Built end-to-end by Elpis.",
         Inches(0.7), Inches(6.15), Inches(11.9), Inches(0.5),
         size=16, color=TEXT_CAPTION, italic=True, align="center")

speaker_notes(s,
    "Establish the platform as one ecosystem across five capabilities — Elpis builds every "
    "layer, software and hardware. Don't deep-dive each pillar; the next slide covers the "
    "acquisition hardware, and the architecture slide shows how they connect. Land the line: "
    "'every layer of the data path, built by us.'")

# ============================================================
# SLIDE 5 — Acquisition hardware (NEW in v7a)
# ============================================================
s = add_blank(prs)
content_chrome(s, 5)

add_text(s, "ACQUISITION HARDWARE",
         Inches(0.7), Inches(0.5), Inches(8), Inches(0.35),
         size=11, color=TEXT_MUTED, letter_spacing=3.0, bold=True)

add_text(s, "Built by Elpis. From the sensor up.",
         Inches(0.7), Inches(0.95), Inches(11.9), Inches(0.9),
         size=34, bold=True, color=TEXT_HEADING, line_spacing=1.1)

# Five product rows — name (left) + one-liner (right), teal tick, separator.
# Specs kept qualitative (no fabricated numbers); E-IDOS framed standalone-today.
hw = [
    ("Edge Gateway", "CONNECTIVITY & EDGE",
     "Ruggedized DIN-rail appliance — a PLC-to-network bridge today (built-in Modbus TCP), the EdgeConnect appliance once Linux ships."),
    ("mDAQ", "DATA ACQUISITION",
     "Multi-channel acquisition for analog and digital signals — connect what controllers won't expose."),
    ("mTracker", "ASSET INTELLIGENCE",
     "Continuous machine-run telemetry — even on un-connected legacy assets."),
    ("VAS", "CONDITION MONITORING",
     "Real-time vibration capture and analysis for rotating machinery — early warning, not hindsight."),
    ("E-IDOS", "CONDITION MONITORING",
     "Hydraulic & lubrication oil-health monitoring — sensor-agnostic, Elpis-built controller. Standalone today."),
]
row_y0 = 2.05
row_h = 0.72
for i, (name, cat, desc) in enumerate(hw):
    y = Inches(row_y0 + i * row_h)
    # teal tick
    add_rect(s, Inches(0.7), y + Inches(0.06), Inches(0.06), Inches(0.5), fill_rgb=BRAND_TEAL)
    # product name
    add_text(s, name, Inches(0.95), y, Inches(3.0), Inches(0.4),
             size=19, bold=True, color=TEXT_HEADING)
    # category eyebrow under name
    add_text(s, cat, Inches(0.95), y + Inches(0.38), Inches(3.0), Inches(0.25),
             size=8.5, color=BRAND_TEAL, bold=True, letter_spacing=1.8)
    # one-liner (right column, vertically centered)
    add_text(s, desc, Inches(4.2), y, Inches(8.4), Inches(0.62),
             size=13, color=TEXT_MUTED, anchor="middle", line_spacing=1.25)
    # separator (except last)
    if i < len(hw) - 1:
        add_rect(s, Inches(0.95), y + row_h - Inches(0.06), Inches(11.65), Inches(0.012),
                 fill_rgb=BORDER_SUBTLE)

# Proof band (LOCKED anonymized trust anchors — no customer names)
proof_y = Inches(5.95)
add_rect(s, Inches(0.7), proof_y, Inches(11.9), Inches(0.85),
         fill_rgb=SURFACE_HERO, line_rgb=BORDER_SUBTLE)
add_rect(s, Inches(0.7), proof_y, Inches(0.08), Inches(0.85), fill_rgb=BRAND_TEAL)
add_text(s, "DEPLOYED WHERE FAILURE ISN'T AN OPTION",
         Inches(0.95), proof_y + Inches(0.14), Inches(11.4), Inches(0.3),
         size=10, color=BRAND_TEAL, letter_spacing=2.5, bold=True)
add_text(s, "Deployed in defense and space-agency programs   ·   "
            "Maintenance and AMC providers across India and the Middle East.",
         Inches(0.95), proof_y + Inches(0.44), Inches(11.4), Inches(0.35),
         size=13, color=TEXT_BODY, italic=True)

speaker_notes(s,
    "This is the slide that earns the 'built by Elpis' claim. Five hardware products — the "
    "acquisition layer the software sits on. Don't read every line; pick the one nearest the "
    "prospect's pain (vibration for rotating machinery, oil for hydraulics, mDAQ for "
    "controller-less sensors). The proof band is anonymized on purpose — defense / space / AMC "
    "channel — never name a customer.")

# ============================================================
# SLIDE 6 — The software that runs it (v7b: software depth)
# ============================================================
s = add_blank(prs)
content_chrome(s, 6)

add_text(s, "THE SOFTWARE",
         Inches(0.7), Inches(0.5), Inches(8), Inches(0.35),
         size=11, color=TEXT_MUTED, letter_spacing=3.0, bold=True)
add_text(s, "Two products turn that signal into decisions.",
         Inches(0.7), Inches(0.95), Inches(11.9), Inches(0.9),
         size=34, bold=True, color=TEXT_HEADING, line_spacing=1.1)

_hy = Inches(2.4); _hh = Inches(3.6); _hw = Inches(4.8)
_lx = Inches(0.7); _rx = Inches(7.85)
# Left — EdgeConnect
add_rect(s, _lx, _hy, _hw, _hh, fill_rgb=SURFACE_HERO, line_rgb=BRAND_TEAL, line_w=Pt(1.5))
add_rect(s, _lx + Inches(0.35), _hy + Inches(0.35), Inches(0.4), Inches(0.06), fill_rgb=BRAND_TEAL)
add_text(s, "EdgeConnect", _lx + Inches(0.35), _hy + Inches(0.55), Inches(4.2), Inches(0.7),
         size=34, bold=True, color=TEXT_HEADING)
add_text(s, "Connectivity & Edge — the runtime", _lx + Inches(0.35), _hy + Inches(1.2),
         Inches(4.2), Inches(0.4), size=15, color=TEXT_MUTED, italic=True)
add_rect(s, _lx + Inches(0.35), _hy + Inches(1.7), Inches(4.1), Inches(0.012), fill_rgb=BORDER_SUBTLE)
add_multiline(s, [
    {"text": "Collects from supported CNC, PLC, and instrumentation sources on your floor.", "size": 14, "color": TEXT_BODY},
    {"text": " ", "size": 6},
...
], _lx + Inches(0.35), _hy + Inches(1.85), Inches(4.1), Inches(1.6), line_spacing=1.4)
# Right — EREMOS V2
add_rect(s, _rx, _hy, _hw, _hh, fill_rgb=SURFACE_HERO, line_rgb=BRAND_TEAL, line_w=Pt(1.5))
add_rect(s, _rx + Inches(0.35), _hy + Inches(0.35), Inches(0.4), Inches(0.06), fill_rgb=BRAND_TEAL)
add_text(s, "EREMOS V2", _rx + Inches(0.35), _hy + Inches(0.55), Inches(4.2), Inches(0.7),
         size=34, bold=True, color=TEXT_HEADING)
add_text(s, "Operational Intelligence", _rx + Inches(0.35), _hy + Inches(1.2),
         Inches(4.2), Inches(0.4), size=15, color=TEXT_MUTED, italic=True)
add_rect(s, _rx + Inches(0.35), _hy + Inches(1.7), Inches(4.1), Inches(0.012), fill_rgb=BORDER_SUBTLE)
add_multiline(s, [
    {"text": "Turns that data into OEE, alarms, incidents, and reports.", "size": 14, "color": TEXT_BODY},
    {"text": " ", "size": 6},
    {"text": "Multi-tenant analytics. OEE via Segments. Persistent alarm + incident workflow. "
     ...
     "size": 13, "color": TEXT_MUTED, "italic": True},
], _rx + Inches(0.35), _hy + Inches(1.85), Inches(4.1), Inches(1.6), line_spacing=1.4)
# Arrow between
_ay = _hy + Inches(1.65)
_axs = _lx + _hw + Inches(0.05); _axe = _rx - Inches(0.05)
add_rect(s, _axs, _ay + Inches(0.16), _axe - _axs, Inches(0.06), fill_rgb=BRAND_TEAL)
add_rect(s, _axe - Inches(0.18), _ay + Inches(0.08), Inches(0.22), Inches(0.22),
         fill_rgb=BRAND_TEAL, shape=MSO_SHAPE.RIGHT_TRIANGLE)
add_text(s, "MQTT  ·  OPC UA", _axs, _ay - Inches(0.45), _axe - _axs, Inches(0.4),
         size=11, color=BRAND_TEAL, align="center", letter_spacing=3.0, bold=True)
add_text(s, "Licensed independently. The two software products are the bookends of the "
            "five-capability stack — the hardware sits between them.",
         Inches(0.7), Inches(6.35), Inches(11.9), Inches(0.5),
         size=16, color=TEXT_CAPTION, italic=True, align="center")

speaker_notes(s,
    "The two software products are the bookends of the five capabilities: EdgeConnect is "
    "Connectivity & Edge, EREMOS V2 is Operational Intelligence. The three hardware-led "
    "capabilities sit between them. Sold and licensed independently; deployed together.")

# ============================================================
# SLIDE 7 — Outcomes you can hold us to
# ============================================================
s = add_blank(prs)
content_chrome(s, 7)
...
arch_path = ASSETS_DIR / "architecture-diagram-v2-slide@2x.png"  # v2 = full stack incl. acquisition hardware
if arch_path.exists():
...
# Caption below
add_text(s, "Acquisition hardware and EdgeConnect collect at the edge of each plant; one EREMOS V2 "
            "tenant aggregates many sites. Standard MQTT and OPC UA keep it interoperable with whatever else you run.",
...
reasons = [
    ...
    ("AI architecture: proposals require human confirmation",
     "Roadmap agents are designed never to silently alter the data path. Local-LLM-capable.", True),  # highlight
    ("Pay for the connectivity you actually use",
     "Per-edition packaging with modular per-protocol activation.", False),
    ("One vendor from sensor to dashboard",
     "Acquisition hardware, edge runtime, and intelligence — all built by Elpis.", False),
]
...
add_text(s, "Software editions — hardware scoped per BOM",
...
speaker_notes(s,
    ...
    "pitch. The roadmap is here so we do not blur shipped capability with planned capability — "
    "be explicit about what ships today versus what's on the way.")
