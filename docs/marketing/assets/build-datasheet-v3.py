"""
File:     docs/marketing/assets/build-datasheet-v3.py
...
Outputs:
          docs/marketing/assets/datasheet-v3-a4.pdf       (210x297 mm)
          docs/marketing/assets/datasheet-v3-letter.pdf   (8.5x11 in)
...
Review-pass cadence:
v1 -> v2  Page 2 diagram enlarged ~10%. Page 4 WHY ELPIS breathing room
          expanded; CTA pulled up; empty band below editions collapsed.
v2 -> v3  Page 4 trust-posture trio (Air-gapped / Lapsed license /
          AI proposes) gets subtle additional emphasis — bigger lead
          (10 -> 11 pt), more between-row spacing within the trio
          (42 -> 46 pt), and a small cluster gap (+12 pt) separating
          the trio from the muted supporting two. Stays well within
          "very small refinement only" — the three are emerging as
          category-defining trust signals and the visual hierarchy
          now reflects that without breaking restraint.

v1 and v2 retained as historical reference. v3 is the new canonical.

Run:      python docs/marketing/assets/build-datasheet-v3.py
"""
...
OUT_A4     = ASSETS / "datasheet-v3-a4.pdf"
OUT_LETTER = ASSETS / "datasheet-v3-letter.pdf"
# v2: narrower diagram-only margin so the architecture image grows ~10% wider
M_DIAGRAM = 10 * mm
# v1 -> v2 corrected: use the DARK MASTER (3:2 aspect) for the datasheet.
# v1 originally used the dark master; v2 first tried the slide variant
# (16:9, no caption) to avoid duplicating the caption — but the slide
# variant has more internal padding (content sits inside a wider canvas),
# so the diagram rendered visually SMALLER even though the bounding box
# was wider. Dark master gives a better content-to-image ratio, and the
# caption is part of the artwork itself so no PDF caption is needed below.
ARCH_IMG   = ASSETS / "architecture-diagram-v1-dark@2x.png"
ARCH_RATIO = 4800 / 3200  # 3:2 aspect of the dark master (= 1.5)
...
def page2_platform(c, pw, ph):
    ...
    # Image — 3:2 aspect (dark master); v1 -> v2 enlarged ~10% wider and
    # ~10% taller by using a narrower diagram-only margin (10 mm vs the
    # 18 mm standard text margin). The dark master has its caption built
    # into the artwork — no separate PDF caption needed.
    img_w = pw - 2 * M_DIAGRAM
    img_h = img_w / ARCH_RATIO  # 3:2 = 1.5
    img_x = M_DIAGRAM
    img_y = arch_label_y - 18 - img_h
    if ARCH_IMG.exists():
        c.drawImage(str(ARCH_IMG), img_x, img_y, width=img_w, height=img_h,
                    preserveAspectRatio=True, mask='auto')
    else:
        draw_rect(c, img_x, img_y, img_w, img_h, fill=HERO,
    ...
    # Caption is part of the dark master artwork — no PDF caption rendered.
...
def page4_close(c, pw, ph):
    ...
    # ----- WHY ELPIS -----
    # v1 -> v2: significantly more breathing room — section gap above
    # 24 -> 38, label-to-first-row 22 -> 36, between-row spacing 30 -> 42.
    # The CTA also pulls up to M_B+70 (from +40) so the empty band
    # between editions/roadmap and the closing CTA collapses to a
    # deliberate rest, not a void.
    why_top = aud_top - 5 * 18 - 38
    ...
    # v2 -> v3: trust-posture trio gets subtle additional emphasis.
    # Iterate with per-row spacing so the trio (highlight=True) uses
    # 46pt between rows, a cluster gap of 12pt separates trio from
    # the muted supporting two, and trio leads are 11pt vs muted 10pt.
    diff_top = why_top - 36
    y = diff_top
    for i, (lead, sub, highlight) in enumerate(differentiators):
        ...
        draw_text(c, lead, M_L + 14, y - 8,
                  font=FONT_BOLD, size=lead_size, color=lead_color)
        draw_text(c, sub, M_L + 14, y - 20,
                  font=FONT_REG, size=8.5, color=TEXT_MUTED)
        # Advance to next row with appropriate spacing
        if i < len(differentiators) - 1:
            next_highlight = differentiators[i + 1][2]
            if highlight and not next_highlight:
                # Cluster gap — trio to muted transition
                y -= 46 + 12
            elif highlight:
                y -= 46   # trio-to-trio (slightly wider than muted)
            else:
                y -= 42   # muted-to-muted

    # ----- EDITIONS & ROADMAP -----
    # v3 inner-loop totals: 2*46 (trio rows 0-2) + 58 (cluster gap)
    # + 42 (muted rows 3-4) + 22 (last card height) + 28 (cushion) = 240
    ed_top = diff_top - 240
    ...
    # ----- NEXT STEP -----
    # v1 -> v2: CTA pulled up 30pt (M_B+40 -> M_B+70) so the empty band
    # above it shrinks. Bottom-of-page margin to footer chrome stays
    # comfortable.
    cta_y = M_B + 70
    cta_h = 78
