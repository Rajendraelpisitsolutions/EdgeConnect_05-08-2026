"""
File:     docs/marketing/assets/build-pitch-deck-v4.py
Purpose:  Generate the executive pitch deck (pitch-deck-v4.pptx) for the
...
Output:   docs/marketing/assets/pitch-deck-v4.pptx
...
v3 -> v4 refinements (third ChatGPT review pass, 2026-05-24):
  Slide 6  - Replaced em-dash placeholder mock with a stylized spreadsheet
             artifact: filename header, 5-column table (Time/Op/Mc/OEE/
             Notes) with 7 rows of synthesized shift-handover data. Two
             "broken" rows are subtly emphasized via brighter text
             (TEXT_BODY vs TEXT_MUTED) — staying in teal-only palette,
             no red/amber. Communicates operational chaos through CONTENT
             rather than color. Closes the biggest remaining visual gap
             flagged in the v3 review.
  Slide 11 - PROFESSIONAL tier visual emphasis: brighter card fill
             (#2E343B vs #2A2F36 siblings), thicker teal stroke
             (Pt 2.0 vs Pt 1.0), tier name size bumped 15 -> 17.
             Reinforces that PROFESSIONAL is the center-of-gravity SKU.

Still deferred (docs/sessions/2026-05-24-pitch-deck-followups.md):
  Slide 6  - Full visual maturity — replace the synthesized table with
             real stylized artifacts (blurred customer screenshots,
             operator handwriting, scanned shift logs) when those become
             available. v4 closes the placeholder-y feel; real artifacts
             would push it further.

v1, v2, v3 retained as historical reference.

Run:      python docs/marketing/assets/build-pitch-deck-v4.py
"""
...
OUT_PPTX = ASSETS_DIR / "pitch-deck-v4.pptx"
