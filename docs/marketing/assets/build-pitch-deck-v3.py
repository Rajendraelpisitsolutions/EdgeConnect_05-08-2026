"""
File:     docs/marketing/assets/build-pitch-deck-v3.py
Purpose:  Generate the executive pitch deck (pitch-deck-v3.pptx) for the
...
Output:   docs/marketing/assets/pitch-deck-v3.pptx

v1 -> v2 refinements (first ChatGPT review pass, 2026-05-24):
  Slide 3  - Audiences reordered to lead with strongest strategic wedges
             (CNC / Brownfield / Multi-site first). Top 3 get slightly more
             visual weight; bottom 2 subtly less prominent.
  Slide 7  - "COMING" roadmap line visually subordinated.
  Slide 11 - Checkbox items per edition reduced 4 -> 3. More whitespace.
             RECOMMENDED ribbon slightly toned down.

v2 -> v3 refinements (second ChatGPT review pass, 2026-05-24):
  Slide 9  - Highlight THREE flagship differentiators instead of two — adds
             "Air-gapped factories are first-class" to the existing pair
             ("Lapsed license" + "AI proposes"). The three together form
             the platform's "trust posture cluster" — the most distinctive
             credibility wins competitors can't replicate without rewriting
             their products. Speaker notes updated to reflect all three.
  Slide 12 - CTA wording changed: "Book a scoping call" -> "Scope your
             first machine". The new CTA mirrors slide 10's incremental
             adoption narrative (cell -> line -> plant -> fleet) and
             reinforces the deck's strongest commercial story:
             coexistence and incremental adoption, not platform replacement.

Deferred to a future visual-maturity pass (docs/sessions/2026-05-24-pitch-deck-followups.md):
  Slide 6  - Replace placeholder spreadsheet mock with stylized operational
             artifacts (blurred fragments, operator notes, timestamp
             inconsistencies). Strategically the slide is excellent; the
             visual execution can mature once real artifacts are available.

v1 and v2 retained as historical reference.

Run:      python docs/marketing/assets/build-pitch-deck-v3.py
"""
...
OUT_PPTX = ASSETS_DIR / "pitch-deck-v3.pptx"
...
for i, (name, sub, modules) in enumerate(editions):
    ...
    if is_pro:
        ...
        add_text(s, "RECOMMENDED", x + Inches(0.5), ed_y - Inches(0.10), Inches(1.3), Inches(0.24),
                 ...
                 letter_spacing=2.0)
    add_text(s, name, x + Inches(0.3), ed_y + Inches(0.28), ed_w - Inches(0.5), Inches(0.4),
    ...
    for j, m in enumerate(modules):
        my = ed_y + Inches(1.42 + j * 0.44)  # was 1.3 + j*0.36
...
add_text(s, "Connectivity modules  ·  FOCAS2  ·  MT-LINKi  ·  MTConnect  ·  Brother HTTP  "
         ...
         Inches(0.7), Inches(5.05), Inches(11.9), Inches(0.4),
         size=11, color=TEXT_MUTED, letter_spacing=1.5, align="center", italic=True)

# Roadmap divider + second headline (pushed down 0.15" for more breathing)
add_rect(s, Inches(0.7), Inches(5.65), Inches(11.9), Inches(0.012), fill_rgb=BORDER_SUBTLE)

add_text(s, "ON THE ROADMAP",
         Inches(0.7), Inches(5.85), Inches(8), Inches(0.3),
