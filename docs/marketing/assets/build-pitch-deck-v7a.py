"""
File:     docs/marketing/assets/build-pitch-deck-v7a.py
Purpose:  *** ARCHIVED — FALLBACK ONLY (2026-06-05) ***
          The CANONICAL deck is build-pitch-deck-v7.py (the promoted v7b
          lineage). This 13-slide "evolve" variant is retained only as a shorter
          fallback if a tighter deck is ever demanded — do NOT use it as the
          master sales deck.
          Generate the (fallback) executive pitch deck (pitch-deck-v7a.pptx) for
          the Elpis Industrial Intelligence Platform.
          VARIANT A — "evolve": keeps the proven v6 spine, reframes the
          solution slide to "One platform. Five capabilities.", and adds a
          dedicated acquisition-hardware slide. See build-pitch-deck-v7.py for
          the canonical full ecosystem rebuild.
...
Output:   docs/marketing/assets/pitch-deck-v7a.pptx
...
v6 -> v7a (Track A hardware-story expansion, VARIANT A — "evolve"):
  Folds the locked 5-capability ecosystem story (homepage v2 / datasheet v5)
  into the proven v6 spine with minimal disruption:
    - Slide 4 reframed "Two products. One platform." -> "One platform. Five
      capabilities." (the 5-pillar strip: Connectivity & Edge, Data Acquisition,
      Asset Intelligence, Condition Monitoring, Operational Intelligence).
    - NEW slide 5 "Acquisition hardware — built by Elpis" (Edge Gateway, mDAQ,
      mTracker, VAS, E-IDOS) + the LOCKED anonymized proof anchors (defense /
      space-agency; AMC across India & the Middle East).
    - Slide "Why Elpis" gains one ecosystem differentiator (built end-to-end).
  Deck grows 12 -> 13 slides. Protocol reality stays correct from v6 (S7 +
  OPC UA Client shipped; MT-LINKi roadmap). Hardware specs are kept high-level /
  qualitative (no fabricated numbers); E-IDOS framed standalone-today.

v1-v6 retained as historical reference. v7a/v7b are the hardware-story variants.

Run:      python docs/marketing/assets/build-pitch-deck-v7a.py
"""
...
OUT_PPTX = ASSETS_DIR / "pitch-deck-v7a.pptx"
...
def content_chrome(slide, slide_num, total=13):
...
add_text(s, "One platform. Five capabilities.",
         Inches(0.7), Inches(0.95), Inches(11.9), Inches(0.9),
...
# ============================================================
# SLIDE 6 — Outcomes you can hold us to
# ============================================================
s = add_blank(prs)
content_chrome(s, 6)
