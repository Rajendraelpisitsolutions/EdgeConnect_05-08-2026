<!--
File:        docs/marketing/solution-precision-manufacturing-v1.md
...
positioning (locked in CNC v2): "emphasize tolerance/quality angle;
OEE accountability across mixed-vendor cells; tighter on the 'high-mix'
operational reality."

Audience overlaps significantly with the CNC machining page but tilts
toward Tier-2/Tier-3 supplier production managers and quality managers
under stricter customer OEE accountability. The category is deliberately
broad (covers automotive parts, component manufacturing, specialized
machining, mid-volume precision work) without overclaiming specific
...
Source narratives:
  ...
  - docs/marketing/solution-cnc-machining-v2.md (template source — heaviest inheritance)
...
Strategic frame: this page is the OEE-accountability-pressured version of
CNC machining. The buyer has the same controller mix as a general CNC
shop, but is under more scrutiny from customers and corporate about the
defensibility of the numbers. "OEE you can defend" is the central
positioning.
-->

# Precision Manufacturing — Solution Page Copy v1
...
>
> Native FOCAS2, MT-LINKi, MTConnect, Brother HTTP, Modbus TCP across high-mix, mixed-vendor production cells. Every input — cycle time, parts count, alarm state, tool wear — collected directly from the controller and timestamped at the edge.
>
...
- The "high-mix, mixed-vendor production cells" phrasing signals operational fit specifically for shops running multiple product families across mixed-generation controllers.
...
> - **Multi-tenant by design** — one platform across multiple sites; per-customer dashboards if your shop reports to multiple Tier-1 customers separately.
...
> - **"Do we still have to maintain separate quality records?"** The platform handles the production-signal side. Inspection-equipment data can flow in via Modbus TCP or MQTT if your quality systems publish that way; otherwise it stays in your existing quality system and the platform handles production-side OEE inputs. Less duplication, fewer fragmented records.
...
| §4 What's included | 290 | Two sub-sections, dense feature lists |
...
| **Total page copy** | **~1,650 words** | Plus diagram + visual elements |

Comparable to the brownfield page (~1,600). The shared vocabulary and structural inheritance from CNC and brownfield kept the precision page tighter than multi-site or OEM despite covering similar topical breadth.
...
- **Industry-specific certification claims** (IATF 16949 for automotive, AS9100 for aerospace, etc.) — page covers precision manufacturing broadly without claiming compliance with industry frameworks the platform hasn't been formally certified against
...
- [ ] No industry-certification claims (IATF, AS9100, ISO 9001, etc.) — page positions broadly without claiming framework compliance
...
*Solution page — Precision Manufacturing, v1, 2026-05-24. Derived from CNC machining solution page v2 template + brownfield modernization v2 mixed-vendor pattern. Fifth and final solution page; completes the vertical-page set.*
