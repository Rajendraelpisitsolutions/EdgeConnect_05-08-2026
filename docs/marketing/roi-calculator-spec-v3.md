<!--
File:        docs/marketing/roi-calculator-spec-v3.md
...
Version:     v3 (Track A refresh — protocol correction, refreshed cross-refs,
                 + hardware BOM-elimination bucket)
Date:        2026-06-05
Status:      LOCKED 2026-06-05 (ChatGPT "Approve with changes" — all P0/P1/P2
                 applied: source-backed + avoidable-% BOM rows, Sample/Customer
                 mode gating of defaults, recurring-value-payback vs Year-1-cash-
                 impact split, full Year-1 cost denominator, shipped-protocol
                 checklist, missing formula inputs added). Next: static
                 /roi-calculator mockup (the §10 list is the build/ship gate).
...
Changes from v2 (v3 — Track A refresh):
  - PROTOCOL CORRECTION: the engineering-driver-savings bucket counts only
    Elpis-SHIPPED southbound protocols (FOCAS2, MTConnect, Brother HTTP,
    Modbus TCP, OPC UA Client, Siemens S7). MT-LINKi is on the Elpis roadmap,
    so it is no longer listed as a covered protocol the customer avoids building.
  - NEW BUCKET: hardware BOM-elimination savings (§3.4 + §4.1) — the avoided
    cost of discrete devices the Elpis hardware replaces (separate industrial PC,
    Linux protocol gateway, cellular modem, standalone condition monitor,
    third-party oil-lab / sample contract). Strictly CUSTOMER-SUPPLIED avoided
    costs — the calculator never assumes a hardware price. Traces to the §18.B
    BOM-elimination lists in the product specs.
  - CROSS-REFS refreshed: datasheet v5 -> v5; added the migrated solution specs
    and the 7 product pages as value-bucket sources.
  - E-IDOS value framed honestly: standalone-instrument value only; no EREMOS V2
    streaming value is computed (streaming is roadmap).
  - REPORTING-FORMULA FIX (pre-existing v2 arithmetic error surfaced while
    building the live mockup): reporting labor is now hours/DAY × days/year ×
    supervisors × reduction% × rate (dropped the erroneous × shifts/day that
    double-counted). §5 reporting corrected $50,625 -> $33,750; recurring
    $66,150; Year-1 $134,550; 3-year $266,850.
  The discipline-first philosophy is UNCHANGED — every output is still a function
  of customer-supplied inputs; no fabricated value.
...
Source narratives:
  - docs/marketing/elpis-industrial-intelligence-platform-v5.md
...
Locked-truth sources: see datasheet v5 header. Every benefit the
...
# ROI Calculator — Specification v3
...
The calculator asks for inputs in three core groups — **operating constants**, **current-state pain**, **assumed improvements** — plus an optional fourth, **avoided hardware / BOM line items** (§3.4).

> **Sample mode vs Customer mode (web variant).** In **Sample mode** the calculator shows example math only. In **Customer mode** it produces a customer ROI **only after** the user has entered their own value for each required input — or explicitly accepted a placeholder via an *"I'm using a placeholder"* acknowledgement. No default ever silently becomes a customer result. (The Excel variant simply leaves required cells blank.)
...
| Contribution margin per part | currency | Marginal revenue minus marginal cost. If unknown, use revenue per part × estimated margin % — **25% is a placeholder the user must acknowledge or override, never a silent default** |
...
| Operating days per year | count | Used by the reporting-labor formula; e.g. 250 for 5-day × 50 weeks |
| Throughput conversion factor | 0–1 | Share of recovered downtime that converts to billable output. **Customer-supplied; default blank.** Replaces the old 0.5–0.7 rule of thumb (§4.1) |
...
| Input | Unit | Notes |
|---|---|---|
| Annual unplanned downtime hours per controller | hours/year | Typical range: 50–300; **150 is a placeholder the user must acknowledge or override (flagged in the output), never a silent default** |
| Hours per day spent on manual reporting (per supervisor) | hours/day | Spreadsheet maintenance, shift-handover prep, OEE stitching — total per day, not per shift (avoids double-counting across shifts) |
| Number of shift supervisors involved in reporting | count | Plant-wide |
| Number of southbound protocols currently uncovered | count | Elpis-shipped protocols (FOCAS2 / MTConnect / Brother HTTP / Modbus TCP / OPC UA Client / Siemens S7) that would otherwise need in-house drivers. MT-LINKi is on the Elpis roadmap — don't count it as covered yet. |
| Months of audit prep saved per major audit | months | Optional input for regulated-industry customers |
| Loaded audit-team cost per month | currency/month | Optional — required only if the audit-prep bucket (§4.1) is used |
| Major audits per year | count | Optional — audit frequency for the audit-prep bucket (§4.1) |

**Shipped-protocol checklist (drives the engineering-driver-savings bucket).** Use a checklist, not a free-typed number, so roadmap protocols can't be counted:

| Protocol | Status | Count as avoided in-house driver? |
|---|---|---|
| FOCAS2 | Shipped | ☐ |
| MTConnect | Shipped | ☐ |
| Brother HTTP (southbound source) | Shipped | ☐ |
| Modbus TCP | Shipped | ☐ |
| OPC UA Client | Shipped | ☐ |
| Siemens S7 | Shipped | ☐ |
| FANUC MT-LINKi (REST) | Roadmap | disabled — not counted |
| HTTP sink (northbound) | Roadmap | disabled — not counted |
| TCP sink (northbound) | Roadmap | disabled — not counted |

**Brother HTTP** here is the shipped *southbound* CNC source — distinct from the roadmap *northbound* generic HTTP sink. Only checked **shipped** protocols feed the §4.1 engineering-driver-savings bucket.
...
**The web variant** can pre-fill a conservative middle value (e.g. 10% downtime reduction, 50% reporting reduction), but the result stays marked **SAMPLE** — labelled *"directional estimate based on conservative defaults — adjust for your operation"* — until the user accepts or overrides each default (Customer mode, see §3 intro).

**The Excel variant** leaves these blank and forces the user to enter their own — preserves credibility.

### 3.4 Avoided hardware / BOM line items (customer-supplied — optional)

For deployments that include Elpis acquisition hardware (Edge Gateway, mDAQ, mTracker, VAS, E-IDOS), the customer can list the discrete devices and contracts the Elpis hardware lets them remove from the BOM. **The customer supplies every avoided-cost figure — the calculator never assumes a hardware price.** Each row traces to the BOM-elimination lists on the product pages (§18.B).

| Avoided line item (examples — **labels only**) | Unit avoided cost | Quantity | One-time / recurring | Avoidable % | Customer cost source | Confirmed removed in this scope? |
|---|---|---|---|---|---|---|
| Separate industrial PC (per gateway site) | customer-entered | count | one-time | 100% or partial | quote / invoice / budget / contract / finance estimate | yes / no |
| Separate Linux protocol gateway | customer-entered | count | one-time | … | … | yes / no |
| Separate cellular modem (per remote site) | customer-entered | count | one-time | … | … | yes / no |
| Standalone condition-monitoring unit | customer-entered | count | one-time | … | … | yes / no |
| Third-party oil-lab / sample-analysis contract | customer-entered / year | count | recurring | % of contract displaced | … | yes / no |

> A row is **calculated only** when the customer enters a unit cost, quantity, avoidable %, and cost source — the example labels carry **no default price**. **Do not count the same avoided device twice** ("industrial PC", "Linux protocol gateway", and "edge appliance" may be the same box). If the Elpis deployment does **not** actually remove the item from the customer's BOM or contract (redundancy, compliance, plant standardization), the row stays **$0**. For a recurring contract only partially displaced, **recurring avoided value = annual contract cost × avoidable %**.

Leave this section blank when no Elpis hardware is in scope. Only the customer's own, source-backed avoided costs are summed — see the discipline rule in §7.
...
|---|---|---|
| **Downtime savings** | `Annual unplanned downtime hours × Number of controllers × Downtime reduction % × Parts per hour × Contribution margin × Throughput conversion factor` | Currency / year |
| **Reporting labor savings** | `Hours per day × Days per year × Number of supervisors × Reporting reduction % × Loaded operator labor rate` | Currency / year |
...
| **Hardware BOM-elimination savings (optional)** | `Σ (avoided unit cost × quantity × avoidable %)` over the §3.4 rows the customer entered and confirmed-removed | Currency — one-time and recurring reported separately, by each row's flag |

**Caveat on the downtime formula** — surfaced prominently in the worksheet, not buried in footnotes:

> *This formula assumes recovered downtime translates into productive throughput. Plants with upstream constraints (material, scheduling, changeovers, staffing) or downstream constraints (warehouse / shipping / takt-rate) should set the **throughput conversion factor** input (§3.1, 0–1) to the share of recovered hours that actually convert to billable parts — the downtime bucket is multiplied by it. Capacity-constrained plants commonly land around 0.5–0.7, but it is a **customer-supplied input, not a baked-in rule**.*

This caveat strengthens CFO defensibility — finance reviewers will look for it.

**Caveat on hardware BOM-elimination** — every figure in this bucket is a number the *customer* supplied for what they would otherwise have bought. The calculator does not price Elpis hardware and does not assume any avoided cost. One-time avoidances (industrial PC, modem, standalone monitor) and recurring avoidances (oil-lab / sample contract) are reported separately, never blended into a single number.

### 4.2 Aggregate outputs

- **Total annual recurring value** = Downtime + Reporting + Audit + *recurring* hardware BOM-elimination savings (excludes all one-time items)
- **Total one-time value (Year 1)** = Engineering driver savings + *one-time* hardware BOM-elimination savings
- **Total Year-1 value** = Annual recurring + one-time value
- **Recurring-value payback (months)** = Year-1 platform cost ÷ monthly recurring value. This is the conservative **headline** payback — it excludes all one-time savings.
- **Year-1 cash-impact view** = Year-1 platform cost − one-time value, shown *separately*. This is **not** recurring payback; one-time engineering / BOM savings improve this view but are never folded into the recurring-payback number.
  - ***Year-1 platform cost*** is supplied by Elpis sales — not the calculator — and **includes everything in the quoted Year-1 scope: software, Elpis hardware, implementation, services, and support**. (Counting avoided hardware on the value side while leaving Elpis hardware out of the cost denominator is not allowed.) The calculator outputs the formula; the sales team enters the price during the conversation.
- **3-year cumulative value** = (Annual recurring × 3) + one-time value
...
- **Predictive-maintenance value from condition monitoring (VAS / E-IDOS)** — avoided catastrophic failures, extended component life, and oil-change optimization are real but highly site-specific; not computed. E-IDOS is valued here only through the hardware-BOM bucket (e.g. an avoided oil-lab contract), never through predicted failure avoidance. (E-IDOS is a standalone instrument today; no EREMOS V2 streaming value is computed.) *Predictive-maintenance value may be modeled only in a custom appendix — and only when the customer supplies their own failure history, component-replacement cost, downtime cost, oil-change history, and assumed reduction. It is not part of the standard calculator.*
...
| Hours/day on manual reporting (per supervisor) | 2 |
| Shift supervisors | 3 |
...
- **Downtime savings/year:** 150 hrs × 30 CNCs × 10% × 6 parts/hr × $12 × 1.0 throughput factor = **$32,400/year**
- **Reporting savings/year:** 2 hrs/day × 250 days × 3 supervisors × 50% × $45 = **$33,750/year**
- **Engineering driver savings (Year 1):** 3 protocols × 6 weeks × 40 hrs × $95 = **$68,400 one-time**
- **Annual recurring value:** $32,400 + $33,750 = **$66,150/year**
- **Year-1 total value:** $66,150 + $68,400 = **$134,550**
- **3-year cumulative value:** ($66,150 × 3) + $68,400 = **$266,850**

> *This software-only example includes no Elpis hardware, so the hardware BOM-elimination bucket is $0. A deployment that, for example, replaces a separate industrial PC and a cellular modem at each remote site would add those customer-supplied avoided costs as a one-time bucket — and an avoided oil-lab contract would add a recurring line. Those figures come from the customer, never from the calculator.*

**Payback illustration:** If the Year-1 platform cost for this plant runs *P* (software + any Elpis hardware + implementation + services + support in scope), the **recurring-value payback** is approximately *P ÷ $66,150/year × 12* months. The Year-1 engineering savings of $68,400 are shown **separately** as one-time value — they improve the **Year-1 cash-impact view** but are **not** included in recurring-value payback. **Elpis sales supplies the actual platform cost during scoping** — the calculator outputs the formula, not the price, to avoid creating a pricing anchor before the commercial conversation.
...
- **Sample mode / Customer mode toggle.** Sample mode shows example math; Customer mode produces a customer result only once each required input is entered or its placeholder explicitly accepted (§3 intro). Nonzero hardware-BOM rows display "customer-supplied" beside them.
- **Conservative defaults pre-filled** for the assumed-improvement inputs in Sample mode, with clear "edit me" affordance; in Customer mode each must be confirmed.
...
- **Never assume hardware or avoided-BOM prices.** The hardware BOM-elimination bucket (§3.4 / §4.1) sums only the avoided costs the *customer* enters for devices and contracts they would otherwise have bought. The calculator never prices Elpis hardware and never assumes an avoided amount. When no hardware figures are entered, the bucket is $0 — not a default.
- **Never present a default-driven result as customer ROI.** A placeholder (contribution-margin %, downtime hours, improvement assumptions) becomes part of a customer result only once the user enters their own value or explicitly accepts the placeholder (Sample vs Customer mode, §3). Sample mode is never presented as the customer's number.
- **Never exclude Elpis hardware / services from the cost denominator** while counting avoided hardware as value. Year-1 platform cost is the full quoted Year-1 scope — software, Elpis hardware, implementation, services, support (§4.2).
- **Recurring contracts are pro-rated by avoidable %** — a partially-displaced oil-lab contract counts only the displaced share, never the whole contract (§3.4 / §4.1).
...
| Hours/day on manual reporting (per supervisor) | 0.5–4 | >4: probably double-counting across shifts; <0.5: probably already automated |
...
## 9. Future extensions (out of scope for v3)
...
- [ ] No literal platform-cost number appears anywhere in the worksheet or web variant (preserves pricing-anchor neutrality per §5)
- [ ] **Hardware-BOM rows (§3.4)** require unit cost + quantity + avoidable % + cost source + confirmed-removed flag; example labels carry NO default price; duplicate-device counting is warned
- [ ] **Sample mode vs Customer mode** implemented; no default-driven result is ever presented as customer ROI (placeholders require explicit acknowledgement)
- [ ] **Year-1 platform cost denominator** is defined as the full quoted Year-1 scope (software + Elpis hardware + implementation + services + support); avoided hardware is never counted as value while Elpis hardware is excluded from cost
- [ ] **Recurring contracts pro-rated by avoidable %** (no whole-contract avoidance unless fully displaced)
- [ ] **Shipped-protocol checklist** used (roadmap items disabled; MT-LINKi not counted; Brother HTTP source ≠ northbound HTTP sink)
- [ ] **Recurring-value payback** and **Year-1 cash-impact view** shown as two separate metrics (one-time savings never folded into recurring payback)
- [ ] All formula inputs are visible in §3 (incl. operating days/year, audit-team rate + audit frequency, throughput conversion factor); reporting is hours/**day**, not per shift
...
*ROI Calculator Spec — v3, 2026-06-05 (Track A refresh). Derived from datasheet v5 §"Typical value areas" + the migrated solution specs + the 7 product pages (§18.B BOM-elimination). v3 adds the customer-supplied hardware BOM-elimination bucket and corrects the protocol list (MT-LINKi roadmap; Siemens S7 + OPC UA Client shipped); the discipline-first, no-fabrication philosophy is unchanged. Next: static /roi-calculator web mockup → ChatGPT review → lock.*
