<!--
File:        docs/marketing/solution-cnc-machining-v1.md
...
Audience:    Web designer + developer building the page; the user signing off
             before publication.
Format:      Markdown page copy. Sections follow the website-messaging-
             architecture-v2 §8.1 template:
               1. Hero
               2. The challenge
               3. The Elpis approach
               4. What's included
               5. Common operator questions (additional section for this vertical)
               6. Customer outcomes
               7. What a typical engagement looks like (additional section)
               8. Architecture (vertical-relevant)
               9. CTA
...
Locked-truth sources: see datasheet v4 header. Every claim on this page
traces back to those sources. CNC-specific claims also trace to
shared-knowledge/contracts/cnc-vocabulary.md.

Voice: same as homepage copy v2 — confident-technical, premium-industrial,
outcomes-first, real protocol names as trust signals. CNC-specific
vocabulary (Fanuc, Brother, Mazak, FOCAS2, MT-LINKi, MTConnect) as
authenticity signals throughout.

Two additional sections beyond the website-messaging template:
  - "Common operator questions" — practical, operator-language questions
    the platform answers. Strengthens operational credibility for
    plant-floor buyers.
  - "What a typical engagement looks like" — psychological reassurance
    for buyers worried about deployment disruption.
These extras may not appear in every vertical's page; for CNC machining
they earn their place because this vertical is the platform's strongest
fit and gets the deepest scrutiny.
-->

# CNC Machining — Solution Page Copy v1
...
> - **"Did the night-shift CNC operators run the right program?"** Program execution history captured per controller, per shift.
...
| §3 Elpis approach | 250 | Four bolded-lead paragraphs |
...
| **Total page copy** | **~1,470 words** | Plus diagram + visual elements |

A solution page is allowed to be denser than the homepage — the reader who reaches a solution page has already qualified themselves into a specific vertical and wants depth.
...
*Solution page — CNC Machining, v1, 2026-05-24. Derived from datasheet v4, website messaging architecture v2 §8.1, and CNC vocabulary contract. Establishes the template pattern for the other four solution pages (precision manufacturing, brownfield modernization, OEM machine monitoring, multi-site operations).*
