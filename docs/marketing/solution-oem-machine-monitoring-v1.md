<!--
File:        docs/marketing/solution-oem-machine-monitoring-v1.md
...
positioning (locked in CNC v2): "different audience entirely (equipment
builders, not plant operators); lead with 'ship connected equipment /
diagnose remotely / no truck rolls'; talk about per-customer fleet
identity."

Audience differs significantly from CNC / brownfield / multi-site: the
buyer is now an OEM product manager, OEM service operations director,
OEM head of installed-base / customer success, or OEM engineering
leader thinking about field telemetry feedback. The strategic frame is
"customer-relationship continuity through trust-respecting telemetry,"
not "plant operational continuity." Two completely different sale.
...
Strategic frame: the OEM page positions Elpis as the connectivity stack
OEMs would otherwise have to build themselves — but layered on top of a
customer-trust architecture (offline-first, no phone-home, customer-
controlled routes) that protects the OEM-customer relationship. Most
OEMs already know they need connected equipment; they don't know they
can have it without becoming the customer's adversary on data control.
-->

# OEM Machine Monitoring — Solution Page Copy v1
...
> - **OPC UA Server (optional)** — if the customer wants to read machine data into their own SCADA / MES, EdgeConnect exposes it natively. Co-existence with customer infrastructure.
> - **Offline-capable** — your machine's connectivity does not depend on cloud, on internet, or on always-on remote access.
>
...
> - **"What about customers with strict no-cloud or air-gap policies?"** Supported. EdgeConnect runs offline. Telemetry can be exported on the customer's schedule via approved channels (scheduled MQTT bursts, manual exports, etc.) rather than always-on connections. Customers with the strictest data-sovereignty requirements remain addressable.
...
| **Total page copy** | **~1,820 words** | Plus diagram + visual elements |

Densest of the solution pages — the OEM audience has the most concerns to resolve (service strategy, customer relationships, data control, pricing scale, white-labeling, product engineering feedback) before they'll engage.
...
*Solution page — OEM Machine Monitoring, v1, 2026-05-24. Derived from CNC machining solution page v2 template with significant adaptation for the OEM audience. Fourth of five solution pages — one remaining: precision manufacturing.*
