<!--
File:        docs/marketing/page-resources-spec-v1.md
...
# Resources center — Page Spec v1 (DRAFT) · ResourceHub + ResourceListing shape-setter

> **⚠️ SUPERSEDED — retained as historical reference.** The LOCKED spec is **`page-resources-spec-v2.md`** (ChatGPT Pass 1 approved with no content edits).
...
| Product brochures — Edge Gateway, mDAQ, mTracker, VAS, E-IDOS | Missing / partial | **Request access** | no download URL; "request access" affordance |
| Whitepapers (edge connectivity, canonical data, OEE, condition monitoring) | Missing | **Coming soon** / **Request access** | no download URL |
...
> NOTE (size.sm italic): Product brochures and the whitepaper series are in preparation — request access on their pages and we'll send them as they publish.
...
*Resources center Page Spec **v1 DRAFT** — defines `ResourceHub` (`/resources`) + `ResourceListing` (`/resources/datasheets`, `/brochures`, `/whitepapers`) for design-system v5, with instance copy. Central discipline: honest card state per the memo §3.2 asset inventory (only the platform datasheet + overview deck exist → "Download"; brochures + whitepapers → "Request access" / "Coming soon"); gating is visual-only (Phase 4 wires capture). Cross-links the LOCKED product/platform/capability/solution owners; re-derives none (memo §6). §8 pre-checks the memo §9 acceptance gate (all green). Awaiting Pass 1 ChatGPT review → v2 lock. Cites: phase3-ia-scope-memo-v2 (parent §3.2/§4/§6/§9/§10 Q3); buyer-taxonomy v1; proof-architecture v1 §3/§4/§8; design-system v4 (→ v5).*
