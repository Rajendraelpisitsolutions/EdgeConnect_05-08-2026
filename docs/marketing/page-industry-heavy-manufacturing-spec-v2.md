<!--
File:        docs/marketing/page-industry-heavy-manufacturing-spec-v2.md
Purpose:     LOCKED page spec for /industries/heavy-manufacturing AND the
...
Date:        2026-06-06
Status:      LOCKED. IndustryShell shape-setter; parent for the 5 sibling verticals.

v1 (page-industry-heavy-manufacturing-spec-v1.md) retained as historical reference.

v1 -> v2 changes (4 ChatGPT-review cleanup edits; no structural change):
  - EDIT 1 (§3.5): geography anchor made VERBATIM + standalone — "Operating
    across India and the Middle East." now opens its own sentence (was
    embedded lowercased mid-sentence). Verbatim form confirmed in §3.1 + §3.5.
  - EDIT 2 (§2 table + §10.2): reusable-shape cardinality made explicit —
    Relevant solutions = 3-5 cards, FAQ = 4-6 Q&A; Heavy Manufacturing
    uses 4 and 5 respectively.
  - EDIT 3 (§3.5): softened "never costs you a cycle or a parts count" to
    designed-to-preserve language ("built to preserve … queuing locally and
    replaying in source order on reconnect").
  - EDIT 4 (§3.3): tightened "MTConnect and Brother HTTP for the rest" to
    protocol-specific wording from the locked connectivity spec ("MTConnect
    for open-standard machines, Brother HTTP for Brother machining centers,
    OPC UA Client where a controller exposes it").

Per ChatGPT Pass 1 verdict: "Approved for v2 after four cleanup edits. The
IndustryShell shape is directionally locked: 8 sections, vertical lens,
cross-links to locked owners, category-level proof for Heavy Manufacturing,
defense/space-agency anchor reserved for Defense/Aerospace, §9 acceptance
gate intact." v2 incorporates the 4 edits and is LOCKED as the IndustryShell
shape-setter.

§9 ACCEPTANCE GATE (phase3 memo v2 §9) — pre-checked in §7.
Word-count target: ~1,000-1,400 words page copy. Current: ~1,185 words.
-->

# `/industries/heavy-manufacturing` — Page Spec v2 (LOCKED) + IndustryShell shape-setter
...
8 sections. This table IS the `IndustryShell` shape (formalized in §10). Cardinality of the variable sections is fixed at the shape level: **Relevant solutions = 3-5 cards; FAQ = 4-6 Q&A.** Heavy Manufacturing uses **4** and **5** respectively.
...
| **4** | Relevant solutions (cards → `/solutions/<solution>`) | `light` | Card grid (3-5 cards; HM uses 4) | ~120 |
| **5** | Proof posture for this industry (anonymized cue) | `light-tinted` | Trust-cue block + `/security` + `/customers` cross-link | ~120 |
| **6** | Common questions (vertical-calibrated inline FAQ) | `light` | 4-6 Q&A (HM uses 5), `FAQPage` schema | ~220 |
...
All copy in §3.1-§3.8. **~1,185 words** (within the ~1,000-1,400 vertical-lens target).
...
- [x] Page copy within ~1,000-1,400 words (current ~1,185)
- [x] All 8 IndustryShell sections present (§2)
- [x] §3.1 hero industry-framed; geography anchor verbatim; no defense anchor
...
- [x] §10 IndustryShell shape definition complete for design-system v5 hand-off
- [x] **EDIT 1** — geography anchor verbatim + standalone in §3.1 + §3.5
- [x] **EDIT 2** — cardinality fixed (solutions 3-5 / FAQ 4-6; HM = 4 / 5) in §2 + §10.2
- [x] **EDIT 3** — store-and-forward softened to designed-to-preserve language (§3.5)
- [x] **EDIT 4** — connectivity protocol wording tightened from the locked connectivity spec (§3.3)
- [x] ChatGPT Pass 1 review applied; v2 LOCKED
...
- **§3.1 Hero** — industry-framed headline + sub. Geography anchor ("Operating across India and the Middle East.") reproduced verbatim; optional but true for all today.
...
- **§3.4 Relevant solutions** — selects **3-5** `/solutions/<solution>` cards that fit the vertical. (HM uses 4.)
- **§3.5 Proof posture** — see override rule §10.3.
- **§3.6 FAQ** — **4-6** vertical-calibrated Q&A. (HM uses 5.)
...
*`/industries/heavy-manufacturing` Page Spec **v2 LOCKED 2026-06-06** after Pass 1 ChatGPT review ("approved for v2 after four cleanup edits… IndustryShell shape directionally locked"). First Phase 3 per-page spec + the `IndustryShell` shape-setter. 8-section vertical-lens layout (§2/§10) the other 5 verticals inherit; cardinality fixed (solutions 3-5, FAQ 4-6). Category-level proof + verbatim geography anchor (defense/space-agency anchor reserved for defense/aerospace per memo §3.1, override §10.3). Cross-links the LOCKED `/capabilities/<pillar>`, `/solutions/<solution>`, `/architecture`, `/platform`, `/security` owners; re-derives none (memo §6). §7 pre-checks the memo §9 acceptance gate (all green). ~1,185 words within the ~1,000-1,400 vertical-lens target. v1 -> v2 applied 4 ChatGPT-review edits: geography anchor verbatim+standalone (§3.5); cardinality made explicit (§2/§10.2); store-and-forward softened to designed-to-preserve (§3.5); connectivity protocol wording tightened (§3.3). Next: 5 sibling vertical specs parallelize against §10 (defense + aerospace get extra anchor review). Cites: phase3-ia-scope-memo-v2 (parent); buyer-taxonomy v1 §2.2/§2.3; proof-architecture v1 §3/§4/§8; positioning v3 §4; page-capabilities-connectivity-edge-spec v1 (protocol wording); design-system v4 (→ v5 §25 IndustryShell).*
