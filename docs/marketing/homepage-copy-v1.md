<!--
File:        docs/marketing/homepage-copy-v1.md
...
Hero copy: three variations provided in §1 so the user can pick the
strongest before publication. All other sections have one version.
...
# Homepage Copy — Production v1

**Every section of the elpisitsolutions.com homepage, in the words that should appear on the page.**

This is the production-grade copy a designer drops into wireframes and a developer drops into HTML. Structural choices are locked by the [website messaging architecture v2](website-messaging-architecture-v2.md) §4 — this document fills in the words.
...
### Copy — Variation A (recommended default)
...
### Copy — Variation B (more outcome-led)
...
### Copy — Variation C (more technical, IT-leaning)
...
- The hero must answer "what is this?" in under 5 seconds. All three variations do that.
- **Recommend Variation A** because it leads with category (`Unified industrial connectivity and operational intelligence`) and supports with three concrete outcome verbs (`connect / measure / reduce`). Variation B is stronger if Elpis wants to lean into the "respects the data path" tagline as a brand line. Variation C is for an IT-leaning audience and may be too dense for first impression.
- **Pick one variation before the wireframe is built.** Multiple versions in production create A/B testing complexity that v1 doesn't need.
...
>
> **Controller fleets keep getting more mixed.** Adding a new Brother cell next to an existing Fanuc line used to mean adding a new monitoring tool. Today's operations teams refuse to maintain three dashboards for one plant.
>
...
- The card descriptions are deliberately specific (`twenty to a hundred CNCs`, `fifteen-year-old Fanuc 16i/18i`, `ten-plus plants`) — specific numbers signal real customer pattern, not abstract marketing.
...
| §8 Use cases | 130 | 5 cards × 25 words each |
| §9 Customer logos | 25 | Placeholder text |
| §10 Final CTA | 60 | Headline + body + CTAs |
| **Total page copy** | **~860 words** | Plus diagram + visual elements |

860 words is the right density for a premium industrial homepage. Long enough to do the work; short enough to scan in 90 seconds.

---

## What's out of scope for v1 of the copy

- **Localization** — English only; Japanese / German / Mandarin variants are a later deliverable
- **A/B test variants** — three hero options provided for user pick; pick one before publication
...
- [ ] Hero variation picked (A / B / C) and other two retired
...
*Homepage Copy — v1, 2026-05-24. Derived from datasheet v4 and website messaging architecture v2.*
