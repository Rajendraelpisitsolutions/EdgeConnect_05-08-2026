<!--
File:        docs/marketing/design-system-v4.md
Purpose:     Design System v4 — ADDITIVE over v3. v3 §1–§23 are canonical and
             UNCHANGED. v4 adds §24 ProductDetail (software + the §24.B hardware
             variant + the §24.A spec-table content pattern + the §24.3 cross-
             lens preset) — the deepest factual product-page composition that the
             capability/solution pages defer to. SUPERSEDES design-system-
             productdetail-addendum-v1.md, whose provisional "§18" numbering
             collided with v3's §18–§23 globals and is renumbered here to §24.
             Also records two governance reconciliations (hardware pages = Phase
             E; hardware-ecosystem-map §264 no-cert resolution) — see "v4
             governance reconciliations" below.
...
Version:     v4 (2026-06-05) — consolidation: ProductDetail (§24) promoted from
             the locked addendum and renumbered §18→§24 to clear the v3 globals
             collision; the 7 product specs were updated in the same change. The
             history line below is the addendum's own review trail.
             v1 (addendum) — ChatGPT review APPLIED (verdict "Approve with changes";
             ...
             /solutions/edge-connectivity; (A5) §24.A spec-table variants + a
...
Date:        2026-06-04
Status:      LOCKED. §24 (software) + §24.B (hardware) both LOCKED 2026-06-04;
             promoted into Design System v4 (renumbered from §18) 2026-06-05.
             §24 (software) — user go-ahead after ChatGPT
             review) — GOVERNING ProductDetail shape until folded into a
             design-system v4 consolidation. /edgeconnect = shape-setter;
             /eremos-v2 inherits §24. §24.B (hardware variant) LOCKED 2026-06-04
             (validated by the /edge-gateway shape-setter + two ChatGPT passes).
             §24.B locks the cert/IP + Phase-E resolutions (see §24.B.0);
...
a §24.B hardware variant extends this shape.
...
ONE new content pattern — the §24.A spec-table — which is a styled `<table>`, not
...
# Elpis DXP — Design System v4 — adds §24 `ProductDetail`

**Design System v4 is additive over v3.** v3 §1–§23 are canonical and unchanged; v4 adds **§24 `ProductDetail`** — the reusable page composition for Phase E product pages, and the deepest factual product surface (protocol/spec depth, semantic modes, security profiles, editions, system requirements, hardware specs) that the capability and solution pages defer to it.

---

## v4 — what this version adds (additive over v3)

v4 adds exactly one thing: **§24 `ProductDetail`** — the **software** variant (§24), the **hardware** variant (§24.B), the **§24.A** spec-table content pattern, and the **§24.3** cross-lens preset. It composes from v3 components and introduces **no new visual primitive** (same governance class as the §16/§17 content patterns).

**Provenance + renumber.** §24 was authored and locked as `design-system-productdetail-addendum-v1.md` under a provisional **§18**. That collided with v3's globals (§18 Motion · §19 Typography · §20 Spacing · §21 Anti-patterns · §22 Coverage map · §23 Sign-off). v4 resolves it by renumbering ProductDetail to **§24** (the next free number after v3's globals) and **supersedes the addendum**. Every former `§18.x` reference is now `§24.x`; the 7 product specs were updated in the same change.

## v4 — governance reconciliations

Two open governance items, resolved here:

1. **Hardware product pages are PHASE E** (not "Phase 3"). design-system v3 line 278 called hardware product pages "Phase 3"; per user direction (2026-06-04) they were authored and shipped in **Phase E** (§24.B + the 5 hardware product pages). This note supersedes v3 line 278.
2. **Hardware certifications — no formal claims (resolves hardware-ecosystem-map §264).** The Elpis hardware products carry **no formal third-party certifications currently** (CE / UL / FCC / IEC); products are **IP65 / IP67-compatible** but **not separately certified**; certification, ingress-protection, and site-compliance are handled **case-by-case during BOM scope**. This is the §24.B.0 cert/IP discipline and the resolution of the §264 open question.

---

## 24.0 Where used + buyer

- **Where used:** `/edgeconnect` (shape-setter) and `/eremos-v2` (software products). Hardware products defer to a future §24.B variant.
...
## 24.1 How ProductDetail differs from CapabilityDeepDive and SolutionPanel

| | CapabilityDeepDive (§14) | SolutionPanel (§15) | **ProductDetail (§24)** |
|---|---|---|---|
...
## 24.2 Section structure (software variant) — 11 sections
...
| 4 | **Spec matrix** — the full protocol/coverage matrix | `light` | **§24.A spec-table** | THE defining section. Status column (Available / Roadmap), semantic-mode + security-profile detail. Honors the locked protocol state (P-G). |
...
| 10 | **Related** — cross-lens | `light-tinted` | cross-lens content pattern (§17) | Cards: the capability pillar + the solutions that use the product + `/architecture`. (Product-page preset — see §24.3.) |
...
## 24.A Spec-table content pattern (NEW — content pattern, not a primitive)
...
## 24.3 Cross-lens preset for `/product` pages (proposed §17 addition)
...
## 24.4 Anti-patterns (ProductDetail-specific)
...
- ❌ Introducing a new visual primitive — ProductDetail composes from v3 components + the §24.A spec-table content pattern only.
...
This addendum is a **proposal**. On sign-off (user + ChatGPT review + the `/edgeconnect` static mockup), §24 + §24.A + the §24.3 cross-lens preset fold into **design-system v4** (additive over v3; no v3 component changed). The `/edgeconnect` page spec is the shape-setter; `/eremos-v2` inherits §24. Hardware product pages use **§24.B (below)**.

---

## 24.B `ProductDetail` hardware variant

**LOCKED 2026-06-04** (user go-ahead; validated by the `/edge-gateway` shape-setter + two ChatGPT passes — full review + cert/IP re-review). §24 (software) and §24.B (hardware) are both LOCKED. Covers the 5 hardware products: **Edge Gateway** (Pillar 1), **mDAQ** (Pillar 2), **mTracker** (Pillar 3), **VAS** + **E-IDOS** (Pillar 4). `/edge-gateway` is the §24.B shape-setter.

### 24.B.0 Scope, buyer, certifications, phase
...
### 24.B.1 How §24.B differs from §24 (software)

§24.B keeps the 11-section ProductDetail arc but swaps the software-specific sections for hardware ones, and **removes the certifications/security-mode depth** (no certs; the trust section becomes field-readiness):

| §24 (software) section | §24.B (hardware) section |
|---|---|
...
| Spec matrix (protocol/integration) | **Hardware specifications** (the §24.A spec-table: dimensions, power, enclosure, environmental, connectivity, I/O, mounting) — the defining section |
...
Hero, What-it-is, Architecture, FAQ, Cross-lens, Final CTA carry over from §24 unchanged (hero gets a hardware-relevant visual — a device line-art or a spec-highlight panel, per the §24 hero-visual slot).

### 24.B.2 The hardware spec-table (§24.A variant)

A `Category | Value` (or `Spec | Value`) table for physical specifications. Same §24.A discipline: real `<th scope>`, token-only styling, **no fabricated numbers**, values trace to hardware-ecosystem-map v3. Group rows (Power · Enclosure · Connectivity · I/O · Environmental · Mounting) are encouraged for scannability. **No certifications row.**

### 24.B.3 Anti-patterns (hardware-specific)

- ❌ **Any formal certification claim** (CE / UL / FCC / IEC / "IP65 certified" / "IP67 certified" / "IP-rated" / "certified rugged" / "field certified") unless formal evidence exists for that exact product/configuration. **Allowed:** the IP65 / IP67-*compatible* wording (see §24.B.0). Cert / IP / site-compliance are handled case-by-case during BOM scope; no certification logos/marks unless formally approved.
...
- ❌ A new visual primitive — §24.B composes from v3 components + the §24.A spec-table.

---

*ProductDetail page-shape addendum v1 — **LOCKED 2026-06-04** after ChatGPT review (verdict "Approve with changes"; all refinements applied) + user go-ahead. Defines the software-variant product-detail composition deferred by design-system v3 §15/§25. Reuses v3 components + one new content pattern (§24.A spec-table); introduces no new primitive. Pairs with the /edgeconnect static HTML mockup for visual sign-off. Folds into design-system v4 on sign-off. Cites: design-system-v3 §14/§15/§16/§17/§5.A/§25, page-capabilities-hub-spec-v1 §9, buyer-taxonomy-v1 §2.3/§2.5, hardware-ecosystem-map-v3 (§264 cert open question), elpis-industrial-intelligence-platform-v5 (product facts), 2026-06-04-phase-e-solution-migration-plan (P-A..P-H).*
