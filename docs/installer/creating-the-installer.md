...
# Host (headless variant): files only in the installer
dotnet publish src/ElpisEdgeConnect.Host/ElpisEdgeConnect.Host.csproj \
    -c Release -r win-x64 --self-contained true -o publish/Host
```
...
| `<Files Include="!(bindpath.HostSrc)\**" />` | Harvests the headless Host files (no service). |
...
wix build installer/ElpisEdgeConnect.wxs \
    ...
    -bindpath "HostSrc=$ROOT/publish/Host" \
...
rm -rf publish/Host publish/Management
...
dotnet publish src/ElpisEdgeConnect.Host/ElpisEdgeConnect.Host.csproj             -c Release -r win-x64 --self-contained true -o publish/Host

wix build installer/ElpisEdgeConnect.wxs -arch x64 -ext WixToolset.Util.wixext \
  -bindpath "HostSrc=$ROOT/publish/Host" -bindpath "MgmtSrc=$ROOT/publish/Management" \
  -o publish/ElpisEdgeConnect.msi
