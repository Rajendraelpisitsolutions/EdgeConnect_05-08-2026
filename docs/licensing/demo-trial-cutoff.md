...
### The budget is collection time, not wall-clock

This trips people up when they try to observe the cutoff by hand. The two hours are
**two hours of active data collection**, not two hours of uptime: the counter is
driven by a rise in total observed data points, so it only advances while a source
is actually collecting.

**With no sources configured, the budget never decrements.** It sits at `02:00:00`
indefinitely, and an unlicensed gateway left running overnight will still report a
full budget in the morning. That is correct behaviour, not a stuck timer.

Combined with §3 — where only the source and sink supervisors stop, while the host
and Studio deliberately keep running so a license can still be activated — the
practical consequence is that **demo mode has no dramatic symptom**. The evidence is
the banner and a `NotLoaded` status, not a crash, a shutdown or a stopped UI. Anyone
waiting for the application to visibly die will conclude nothing happened.
