...
public sealed class RuntimeDiagnosticsCollectorTests
{
    ...
    public void SinkLifecycle_DegradedDrainingRecovered_OrderingPinned()
    {
        ...
        // These two assertions used to require the error to SURVIVE recovery,
        // which pinned a real defect rather than a decision: RouteCard renders
        // its fault panel off LastErrorAtUtc (not IsDegraded), and RouteHealth
        // folds a live sink error into the route verdict — so a destination that
        // dropped for 227 ms showed a red "not connected" panel, and marked its
        // route broken, for the rest of the gateway's uptime.
        //
        // Recovery is the END of degraded -> draining -> recovered: reconnected
        // AND drained, with nothing left for the operator to act on. The error
        // is still deliberately retained while degraded or draining, and the
        // degraded entry keeps its error in the event ring, so no history is
        // lost — only the "act on this now" signal is cleared.
        sink.LastError.Should().BeNull("recovery means there is nothing left to act on");
        sink.LastErrorAtUtc.Should().BeNull();
    }

    [Fact]
    public void SinkAdapterRunning_ClearsAdapterLevelError_WhenNoDeliveryFaultOutstanding()
    {
        // The mirror of the OnSinkRecovered bug. An ADAPTER-level fault (a broker
        // that refused the connect) is never followed by a publish-level
        // recovery, because the sink was never publish-degraded — so nothing
        // released it and the route read broken while publishing normally.
        var c = new RuntimeDiagnosticsCollector();
        c.RecordSinkAdapterState("r1", "s1", AdapterState.Failed,
            new AdapterError
            {
                Code = "MQTT.CONNECT_FAILED",
                Category = ErrorCategory.Network,
                Message = "refused",
            });

        c.GetRouteSnapshot("r1")!.Sinks[0].LastError.Should().NotBeNull();

        c.RecordSinkAdapterState("r1", "s1", AdapterState.Running, lastError: null);

        var sink = c.GetRouteSnapshot("r1")!.Sinks[0];
        sink.LastError.Should().BeNull("the broker is connected again and no delivery fault stands");
        sink.LastErrorAtUtc.Should().BeNull();
    }

    [Fact]
    public void SinkAdapterRunning_KeepsError_WhileDeliveryIsStillDegraded()
    {
        // The reason the adapter path cannot clear unconditionally: a broker can
        // be connected while delivery still fails. Blanking the error on a
        // Running adapter would hide a fault that is very much live.
        var c = new RuntimeDiagnosticsCollector();
        c.OnSinkDegraded(DegradedEvt("r1", "s1"));

        c.RecordSinkAdapterState("r1", "s1", AdapterState.Running, lastError: null);

        c.GetRouteSnapshot("r1")!.Sinks[0].LastError
            .Should().NotBeNull("publish-level degradation still owns the error");
    }

    [Fact]
    public void SinkRecovered_ClearsStaleError_SoTheFaultPanelDoesNotStickForever()
    {
        // Mirrors RecordSourceState_RecoveryToRunning_ClearsStaleError on the
        // source side. The absence of this test on the sink side is why the
        // source path was fixed and the sink path was not.
        var c = new RuntimeDiagnosticsCollector();
        c.OnSinkDegraded(DegradedEvt("r1", "s1"));

        c.GetRouteSnapshot("r1")!.Sinks[0].LastError
            .Should().NotBeNull("the error must persist while the sink is degraded");

        c.OnSinkDraining(DrainingEvt("r1", "s1"));

        c.GetRouteSnapshot("r1")!.Sinks[0].LastError
            .Should().NotBeNull("draining is not yet recovered — the fault still stands");

        c.OnSinkRecovered(RecoveredEvt("r1", "s1"));

        var sink = c.GetRouteSnapshot("r1")!.Sinks[0];
        sink.LastError.Should().BeNull();
        sink.LastErrorAtUtc.Should().BeNull();
    }
