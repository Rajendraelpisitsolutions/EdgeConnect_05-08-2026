...
public sealed class BrotherHttpHttpApiTests
{
    ...
    [Theory]
    [InlineData("http://")]      // scheme present but no host
    [InlineData("[bad")]         // unparseable even once http:// is implied
    [InlineData("ftp://cnc")]    // Brother speaks HTTP only
    public void Constructor_UnusableBaseUrl_Throws(string baseUrl)
    {
        Action act = () => _ = new BrotherHttpHttpApi(
            factory: new StubHttpClientFactory(),
            baseUrl: baseUrl,
    ...
    [Theory]
    [InlineData("192.168.5.25", "http://192.168.5.25/")]
    [InlineData("cnc-line-a", "http://cnc-line-a/")]
    [InlineData("192.168.5.25:8080", "http://192.168.5.25:8080/")]
    public void Constructor_BaseUrlWithoutScheme_ImpliesHttp(string baseUrl, string expected)
    {
        // Brother's web-monitoring interface is always plain HTTP on port 80,
        // so a bare address the operator typed must not be a construction
        // failure — the adapter supplies the only scheme it could have meant.
        ...
        api.BaseAddressForTesting.AbsoluteUri.Should().Be(expected);
    }
