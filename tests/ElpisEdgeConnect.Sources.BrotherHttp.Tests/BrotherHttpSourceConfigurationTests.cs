...
public sealed class BrotherHttpSourceConfigurationTests
{
    ...
    // A path fragment is not an address. TrimStart('/') used to eat this single
    // leading slash and hand back host "relative", so a plainly malformed value
    // was accepted here and only failed later against a machine that does not
    // exist. MTConnect's twin normaliser had the identical bug.
