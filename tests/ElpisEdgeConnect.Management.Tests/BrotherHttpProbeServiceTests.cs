...
public sealed class BrotherHttpProbeServiceTests
{
    ...
    public async Task ProbeAsync_InvalidBaseUrl_FailsWithConfigInvalid()
    {
        ...
        var outcome = await service.ProbeAsync(
            // "[bad" is unparseable as a host. Note this used to read "not-a-url",
            // which is now VALID — a bare host name is normalised to
            // http://not-a-url, because operators type plain IPs and host names.
            new BrotherHttpProbeRequest { BaseUrl = "[bad" },
            CancellationToken.None);
    ...
    [Theory]
    [InlineData("192.168.2.110", "http://192.168.2.110")]
    [InlineData("cnc-line-a", "http://cnc-line-a")]
    [InlineData("192.168.2.110:8080", "http://192.168.2.110:8080")]
    public void NormaliseBaseUrl_BareHost_ImpliesHttp(string input, string expected)
    {
        // Brother serves its web-monitoring page over plain HTTP on port 80, so a
        // bare address is unambiguous. Requiring the operator to type "http://"
        // only added a way to fail the probe — see BrotherHttpSourceConfiguration.
        BrotherHttpProbeService.NormaliseBaseUrl(input).Should().Be(expected);
    }
    ...
    [InlineData("[bad")]
    [InlineData("http://[bad")]
    [InlineData("ftp://something")]  // Brother is http(s) only; rejected here rather than as a transport fault mid-probe
    public void NormaliseBaseUrl_Unusable_ReturnsNull(string input)
    {
        BrotherHttpProbeService.NormaliseBaseUrl(input).Should().BeNull();
    }

    [Fact]
    public void NormaliseBaseUrl_Https_IsNotDowngraded()
    {
        BrotherHttpProbeService.NormaliseBaseUrl("https://cnc.local/")
            .Should().Be("https://cnc.local");
    }
