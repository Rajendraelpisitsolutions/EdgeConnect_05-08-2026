...
public class SourceProtocolPickerModelTests
{
    ...
    [Fact]
    public void Resolve_ProfessionalEdition_OffersEverythingExceptOpcUaClient()
    {
        // Professional offers the full source catalogue EXCEPT OPC UA Client
        // (Sony scoping) — the opcua-client tile is hidden entirely, all others
        // stay Available.
        var tiles = SourceProtocolPickerModel.Resolve(_ => true, LicenseEdition.Professional);

        tiles.Should().NotContain(t => t.Key == "opcua-client");
        tiles.Should().HaveCount(SourceProtocolPickerModel.Tiles.Count - 1);
        tiles.Should().OnlyContain(t => t.Status == SourceProtocolTileStatus.Available);
        tiles.Should().Contain(t => t.Key == "s7");
    }

    [Fact]
    public void Resolve_CncProEdition_OffersOnlyFocas2BrotherAndMtconnect()
    {
        // CNC Pro is a CNC-focused bundle: FOCAS2 + Brother HTTP + MTConnect only.
        var tiles = SourceProtocolPickerModel.Resolve(_ => true, LicenseEdition.CncPro);

        tiles.Select(t => t.Key).Should().Equal("focas2", "brother-http", "mtconnect");
        tiles.Should().OnlyContain(t => t.Status == SourceProtocolTileStatus.Available);
    }

    [Fact]
    public void Resolve_TrialPeriodEdition_OffersFullCatalogue()
    {
        // Trial lets the operator try everything.
        var tiles = SourceProtocolPickerModel.Resolve(_ => true, LicenseEdition.TrialPeriod);
        tiles.Should().HaveCount(SourceProtocolPickerModel.Tiles.Count);
    }

    [Fact]
    public void Resolve_EnterpriseEdition_OffersFullCatalogue()
    {
        var tiles = SourceProtocolPickerModel.Resolve(_ => true, LicenseEdition.Enterprise);
        tiles.Should().HaveCount(SourceProtocolPickerModel.Tiles.Count);
    }
