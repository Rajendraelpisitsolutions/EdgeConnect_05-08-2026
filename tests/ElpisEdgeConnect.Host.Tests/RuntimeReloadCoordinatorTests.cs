...
public sealed class RuntimeReloadCoordinatorTests
{
    ...
    // ════════════════════════════════════════════════════════════════
    // Endpoint-state republish + sink→route rebind cascade (5)
    //
    // ONE reported symptom, TWO defects behind it. An operator retyped a
    // source's IP address and port and hit Save. The route card then read
    // "No destinations attached" with a red "Not delivering" badge — while
    // the source was Running, reading, and the buffer was draining normally.
    //
    //   1. Route teardown (phase A1) calls
    //      RuntimeDiagnosticsCollector.RemoveRoute, which drops the ENTIRE
    //      per-route record: source rollup, sink map, buffer gauge, event
    //      rings. Those endpoint entries are written by the SUPERVISORS, and
    //      only when the adapter itself starts/stops — so an endpoint the
    //      reload did NOT touch never pushes again. The rebuilt route came
    //      back with an EMPTY sink map, and zero destinations maps straight
    //      to Down, which the Studio renders as "Not delivering".
    //
    //   2. The rebind cascade only handled sources. A restarted SINK builds a
    //      brand-new ISinkAdapter, but RouteWorker captured the OLD instance
    //      when it started and nothing re-resolves — so the route kept
    //      publishing into a stopped-and-disposed adapter while the fresh
    //      instance was bound to no route at all.
    //
    // These tests read the collector directly. `Sinks` empty in the snapshot
    // IS the "No destinations attached" card, and a null / Created `Source`
    // IS the "no readings arriving" one — see
    // Management/Components/Shared/RouteHealth.Verdict, whose first step is
    // literally `route.Sinks.Count == 0` → Down and whose third is
    // `!IsRunning(route.Source.StateName)` → Down. Host.Tests does not
    // reference the Management project, so the assertions are made one layer
    // below the Blazor component, against the values that feed it.
    //
    // Each test asserts the rebind (outcome.RestartedInstances) AND the
    // republished diagnostics, deliberately: the rebind assertion fails if
    // the sink cascade is removed, the diagnostics assertion fails if the
    // republish is removed, and a test that only asserted the second would
    // pass with BOTH removed — no rebind means no teardown means the stale
    // subtree survives by accident.
    // ════════════════════════════════════════════════════════════════

    /// <summary>
    /// A source whose connection block carries a host + port, so a test can
    /// retype exactly those two fields and change nothing else — the operator
    /// edit that produced the reported symptom.
    /// </summary>
    private static SourceInstanceConfig SrcCfgAt(string id, string host, int port, int pointsPerPoll = 1) => new()
    {
        InstanceId = id,
        ProtocolName = "mock",
        DeviceId = "dev-" + id,
        Enabled = true,
        Connection = JsonSerializer.SerializeToElement(new { host, port, pointsPerPoll }),
    };

    /// <summary>The destination-side mirror of <see cref="SrcCfgAt"/>.</summary>
    private static SinkInstanceConfig SnkCfgAt(string id, string host, int port) => new()
    {
        InstanceId = id,
        ProtocolName = "mock",
        Enabled = true,
        Connection = JsonSerializer.SerializeToElement(new { host, port }),
    };

    [Fact]
    public async Task Reconcile_SourceConnectionEdit_RouteKeepsItsDestinationAttached_AndKeepsDelivering()
    {
        // THE REPORTED REPRO, VERBATIM. Route is up and delivering. The
        // operator retypes ONLY the source's host and port and applies —
        // nothing added, nothing removed, the destination and the route are
        // byte-identical across the two configs. The card must not lose its
        // destination.
        await using var fx = new CoordinatorFixture(MakeConfig());
        fx.Coordinator.Subscribe();

        MockSourceAdapter? liveSource = null;
        fx.Factory.SourceBuilders["plc-1"] = _ =>
        {
            liveSource = new MockSourceAdapter("plc-1") { PointsPerPoll = 1 };
            return FakeSourceReg(liveSource);
        };
        var sink = new MockSinkAdapter("mqtt-1");
        fx.Factory.SinkBuilders["mqtt-1"] = _ => FakeSinkReg(sink);

        var before = MakeConfig(
            sources: new[] { SrcCfgAt("plc-1", "10.20.0.5", 502) },
            sinks: new[] { SnkCfgAt("mqtt-1", "broker.local", 1883) },
            routes: new[] { RouteWithBuffer("r-1", "plc-1", new[] { "mqtt-1" }, maxDepth: 1000) });
        fx.ConfigManager.SimulateApply(before, new[]
        {
            Added(ConfigurationEntityKind.Source, "plc-1"),
            Added(ConfigurationEntityKind.Sink, "mqtt-1"),
            Added(ConfigurationEntityKind.Route, "r-1"),
        });
        var up = await WaitForAsync(
            () => fx.RoutingEngine.RegisteredRouteIds.Contains("r-1") && sink.PublishedCount > 0,
            TimeSpan.FromSeconds(10));
        up.Should().BeTrue("baseline: the route must be delivering before the edit");

        // The edit: same instance ids, same route, same destination — only the
        // source's host and port differ.
        var after = MakeConfig(
            sources: new[] { SrcCfgAt("plc-1", "10.20.0.9", 5020) },
            sinks: new[] { SnkCfgAt("mqtt-1", "broker.local", 1883) },
            routes: new[] { RouteWithBuffer("r-1", "plc-1", new[] { "mqtt-1" }, maxDepth: 1000) });
        var v = fx.ConfigManager.SimulateApply(after, new[]
        {
            Modified(ConfigurationEntityKind.Source, "plc-1"),
        });

        var outcome = await fx.OutcomeRegistry!.WaitForAsync(v, TimeSpan.FromSeconds(10), CancellationToken.None);

        outcome.Should().NotBeNull();
        outcome!.Status.Should().Be(ReloadStatus.Completed);
        outcome.FaultedInstances.Should().BeEmpty();
        outcome.RestartedInstances.Should().Contain("plc-1");
        outcome.RestartedInstances.Should().Contain("r-1", "the source edit cascades a route rebind (M.P2.4)");

        // The card. The outcome is enqueued at the END of the reconcile, after
        // the route bring-up republished its endpoints, so this needs no wait:
        // if the sink map is empty here it is empty for good — the destination
        // adapter was never touched by this reload and will never push again.
        var snapshot = fx.Diagnostics.GetRouteSnapshot("r-1");
        snapshot.Should().NotBeNull("the rebuilt route must be known to the collector");
        snapshot!.Sinks.Should().ContainSingle(
            "a source-only edit must not cost the route its destination — an empty sink map is "
            + "exactly what the Studio renders as \"No destinations attached\" / \"Not delivering\"")
            .Which.AdapterState.Should().Be(AdapterState.Running,
                "the destination adapter kept running throughout; the card must say so");
        snapshot.Source!.State.Should().Be(AdapterState.Running);

        // And it is still actually delivering, not merely reported as such.
        var baseline = sink.PublishedCount;
        var stillDelivering = await WaitForAsync(() => sink.PublishedCount > baseline, TimeSpan.FromSeconds(10));
        stillDelivering.Should().BeTrue(
            $"the route must keep delivering across the edit; source {liveSource?.InstanceId} emitted "
            + $"{liveSource?.EmittedCount} points, sink received {sink.PublishedCount}");
    }

    [Fact]
    public async Task Reconcile_DestinationEdit_LeavesSourceReportedRunning_NotCreated()
    {
        // THE MIRROR. Retype only the DESTINATION's host and port. The source
        // is not touched, so nothing in the supervisor re-pushes its state
        // after the teardown wiped the route's subtree.
        //
        // This source is chatty (the default PointsPerPoll = 1), which is what
        // makes the failure mode subtle rather than absent: the poll loop's
        // RecordSourceObservation re-creates the source entry on the next
        // non-empty batch — but EnsureSource builds it with State = Created,
        // never Running. So the card comes back with a source that reads
        // "Created, no readings arriving" sitting next to a points counter that
        // is visibly ticking up. Asserting the STATE, not merely the entry's
        // existence, is what catches that.
        await using var fx = new CoordinatorFixture(MakeConfig());
        fx.Coordinator.Subscribe();

        var source = new MockSourceAdapter("plc-1") { PointsPerPoll = 1 };
        fx.Factory.SourceBuilders["plc-1"] = _ => FakeSourceReg(source);
        fx.Factory.SinkBuilders["mqtt-1"] = _ => FakeSinkReg(new MockSinkAdapter("mqtt-1"));
        ...
        // Only the destination's connection block changes.
        var after = MakeConfig(
            sources: new[] { SrcCfgAt("plc-1", "10.20.0.5", 502) },
            sinks: new[] { SnkCfgAt("mqtt-1", "broker-2.local", 8883) },
            routes: new[] { RouteWithBuffer("r-1", "plc-1", new[] { "mqtt-1" }, maxDepth: 1000) });
        var v = fx.ConfigManager.SimulateApply(after, new[]
        {
            Modified(ConfigurationEntityKind.Sink, "mqtt-1"),
        });
        ...
        outcome.Should().NotBeNull();
        outcome!.FaultedInstances.Should().BeEmpty();
        outcome.RestartedInstances.Should().Contain("mqtt-1");
        outcome.RestartedInstances.Should().Contain("r-1",
            "a rebuilt destination leaves the running route holding a disposed adapter, so the "
            + "route must be rebound — the sink-side mirror of the M.P2.4 source cascade");

        var snapshot = fx.Diagnostics.GetRouteSnapshot("r-1");
        snapshot.Should().NotBeNull();
        snapshot!.Source.Should().NotBeNull("the untouched source must survive the destination edit");
        snapshot.Source!.State.Should().Be(AdapterState.Running,
            "the source never stopped reading; a Created state here is the "
            + "\"source is created, no readings arriving\" card the operator was shown");
        snapshot.Sinks.Should().ContainSingle().Which.AdapterState.Should().Be(AdapterState.Running);
    }

    [Fact]
    public async Task Reconcile_DestinationEdit_QuietSource_StateIsRestoredByRepublishAlone()
    {
        // THE ONE THAT ACTUALLY PROVES IT. Same destination-only edit as the
        // mirror above, but the source reads NOTHING — points-per-poll 0, a
        // machine that is connected and polling and simply has no new values
        // (a Modbus source between scheduled scans, an idle CNC).
        //
        // Why this matters more than the chatty version: the source supervisor
        // pushes state only at Initialize / Start / Stop boundaries and pushes
        // observations only for NON-EMPTY batches. A chatty source therefore
        // re-creates its own diagnostics entry on the very next poll, and a
        // test written with one can pass on that accident even with the
        // coordinator's republish deleted — it would only ever have caught the
        // wrong STATE, never the missing entry. A source that reads nothing
        // re-pushes nothing, ever, so RepublishEndpointStatesForRoute is the
        // ONLY thing in the system that can put the source back on the card.
        // The assertion below is therefore true if and only if the republish
        // ran.
        await using var fx = new CoordinatorFixture(MakeConfig());
        fx.Coordinator.Subscribe();

        var quiet = new MockSourceAdapter("plc-quiet") { PointsPerPoll = 0 };
        fx.Factory.SourceBuilders["plc-quiet"] = _ => FakeSourceReg(quiet);
        fx.Factory.SinkBuilders["mqtt-1"] = _ => FakeSinkReg(new MockSinkAdapter("mqtt-1"));

        var before = MakeConfig(
            sources: new[] { SrcCfgAt("plc-quiet", "10.20.0.5", 502, pointsPerPoll: 0) },
            sinks: new[] { SnkCfgAt("mqtt-1", "broker.local", 1883) },
            routes: new[] { RouteWithBuffer("r-1", "plc-quiet", new[] { "mqtt-1" }, maxDepth: 1000) });
        fx.ConfigManager.SimulateApply(before, new[]
        {
            Added(ConfigurationEntityKind.Source, "plc-quiet"),
            Added(ConfigurationEntityKind.Sink, "mqtt-1"),
            Added(ConfigurationEntityKind.Route, "r-1"),
        });
        await WaitForAsync(() => fx.RoutingEngine.RegisteredRouteIds.Contains("r-1"), TimeSpan.FromSeconds(10));

        fx.Diagnostics.GetRouteSnapshot("r-1")!.Source.Should().NotBeNull(
            "baseline: the supervisor reported the source once, at start");
        ...
        outcome!.FaultedInstances.Should().BeEmpty();
        outcome.RestartedInstances.Should().Contain("r-1",
            "the destination rebuild must rebind the route — that rebind is what wipes the "
            + "diagnostics subtree, and therefore what makes the republish load-bearing");

        var snapshot = fx.Diagnostics.GetRouteSnapshot("r-1");
        snapshot.Should().NotBeNull();
        snapshot!.Source.Should().NotBeNull(
            "a source that reads nothing never re-pushes its own state, so only the coordinator's "
            + "republish can restore it after the route teardown dropped the whole subtree");
        snapshot.Source!.State.Should().Be(AdapterState.Running);
        snapshot.Source.SourceInstanceId.Should().Be("plc-quiet");
        snapshot.Source.PointsObserved.Should().Be(0L,
            "the point of this source is that it has read nothing — if this is non-zero the "
            + "test has quietly become the chatty one and no longer proves anything");
        snapshot.Sinks.Should().ContainSingle().Which.AdapterState.Should().Be(AdapterState.Running);
    }

    [Fact]
    public async Task Reconcile_RestartSink_CascadesRebindOfDependentRoute()
    {
        // Defect 2 on its own terms, asserted through data rather than
        // bookkeeping: after a destination edit, points must reach the NEW
        // adapter instance. Without the cascade the route keeps the instance
        // RouteWorker captured at start — the one A3 just stopped and disposed
        // — so the new instance's PublishedCount stays at 0 forever while the
        // dead one's keeps climbing.
        await using var fx = new CoordinatorFixture(MakeConfig());
        fx.Coordinator.Subscribe();
        ...
        var sinks = new List<MockSinkAdapter>();
        fx.Factory.SinkBuilders["mqtt-1"] = _ =>
        {
            var built = new MockSinkAdapter("mqtt-1");
            lock (sinks) sinks.Add(built);
            return FakeSinkReg(built);
        };
        ...
        var up = await WaitForAsync(
            () => { lock (sinks) return sinks.Count == 1 && sinks[0].PublishedCount > 0; },
            TimeSpan.FromSeconds(10));
        up.Should().BeTrue("baseline: the first destination instance must be publishing");

        var edited = MakeConfig(
            sources: new[] { SrcCfgAt("plc-1", "10.20.0.5", 502) },
            sinks: new[] { SnkCfgAt("mqtt-1", "broker-2.local", 8883) },
            routes: new[] { RouteWithBuffer("r-1", "plc-1", new[] { "mqtt-1" }, maxDepth: 1000) });
        var v = fx.ConfigManager.SimulateApply(edited, new[]
        {
            Modified(ConfigurationEntityKind.Sink, "mqtt-1"),
        });
        ...
        outcome.RestartedInstances.Should().Contain("mqtt-1");
        outcome.RestartedInstances.Should().Contain("r-1",
            "a rebuilt sink instance is bound to nothing until the route is rebuilt around it");

        MockSinkAdapter replacement;
        lock (sinks)
        {
            sinks.Should().HaveCount(2, "the destination edit rebuilds the adapter");
            replacement = sinks[1];
        }

        var rebound = await WaitForAsync(() => replacement.PublishedCount > 0, TimeSpan.FromSeconds(10));
        rebound.Should().BeTrue(
            "the route must publish into the REPLACEMENT adapter; if this instance never receives a "
            + "point the route is still holding the stopped-and-disposed original");
    }

    [Fact]
    public async Task Reconcile_RestartSink_DoesNotCascadeRebindOfUnrelatedRoute()
    {
        // NEGATIVE CONTROL for both cascades. Two independent routes; only
        // r-1's destination is edited. r-2 shares nothing with it and must be
        // left strictly alone — an over-broad rebind (every registered route,
        // or every route on any sink action) would restart r-2 too, tearing
        // down a healthy route's worker, buffer and diagnostics for no reason.
        // Asserted on the reload outcome, which is a COMPLETE record of the
        // reconcile, so "did not happen" needs no sleep to observe.
        await using var fx = new CoordinatorFixture(MakeConfig());
        fx.Coordinator.Subscribe();

        fx.Factory.SourceBuilders["plc-1"] = _ => FakeSourceReg(new MockSourceAdapter("plc-1"));
        fx.Factory.SourceBuilders["plc-2"] = _ => FakeSourceReg(new MockSourceAdapter("plc-2"));
        fx.Factory.SinkBuilders["mqtt-1"] = _ => FakeSinkReg(new MockSinkAdapter("mqtt-1"));

        var snk2BuildCount = 0;
        fx.Factory.SinkBuilders["mqtt-2"] = _ =>
        {
            Interlocked.Increment(ref snk2BuildCount);
            return FakeSinkReg(new MockSinkAdapter("mqtt-2"));
        };

        var before = MakeConfig(
            sources: new[] { SrcCfgAt("plc-1", "10.20.0.5", 502), SrcCfgAt("plc-2", "10.20.0.6", 502) },
            sinks: new[] { SnkCfgAt("mqtt-1", "broker.local", 1883), SnkCfgAt("mqtt-2", "broker.local", 1884) },
            routes: new[]
            {
                Route("r-1", "plc-1", new[] { "mqtt-1" }),
                Route("r-2", "plc-2", new[] { "mqtt-2" }),
            });
        fx.ConfigManager.SimulateApply(before, new[]
        {
            Added(ConfigurationEntityKind.Source, "plc-1"),
            Added(ConfigurationEntityKind.Source, "plc-2"),
            Added(ConfigurationEntityKind.Sink, "mqtt-1"),
            Added(ConfigurationEntityKind.Sink, "mqtt-2"),
            Added(ConfigurationEntityKind.Route, "r-1"),
            Added(ConfigurationEntityKind.Route, "r-2"),
        });
        await WaitForAsync(
            () => fx.RoutingEngine.RegisteredRouteIds.Contains("r-1")
                && fx.RoutingEngine.RegisteredRouteIds.Contains("r-2"),
            TimeSpan.FromSeconds(10));

        var after = MakeConfig(
            sources: new[] { SrcCfgAt("plc-1", "10.20.0.5", 502), SrcCfgAt("plc-2", "10.20.0.6", 502) },
            sinks: new[] { SnkCfgAt("mqtt-1", "broker-2.local", 8883), SnkCfgAt("mqtt-2", "broker.local", 1884) },
            routes: new[]
            {
                Route("r-1", "plc-1", new[] { "mqtt-1" }),
                Route("r-2", "plc-2", new[] { "mqtt-2" }),
        ...
        outcome!.FaultedInstances.Should().BeEmpty();
        outcome.RestartedInstances.Should().Contain("mqtt-1");
        outcome.RestartedInstances.Should().Contain("r-1", "r-1 binds the rebuilt destination");
        outcome.RestartedInstances.Should().NotContain("r-2",
            "r-2 binds neither the rebuilt destination nor a restarted source; rebinding it would "
            + "tear down a healthy route the operator never touched");
        outcome.RestartedInstances.Should().NotContain("mqtt-2");
        outcome.RestartedInstances.Should().NotContain("plc-1");
        outcome.RestartedInstances.Should().NotContain("plc-2");
        outcome.AppliedInstances.Should().BeEmpty("nothing was added by this apply");

        snk2BuildCount.Should().Be(1, "the untouched destination must not be rebuilt");
        fx.RoutingEngine.RegisteredRouteIds.Should().Contain("r-2");
    }
}
