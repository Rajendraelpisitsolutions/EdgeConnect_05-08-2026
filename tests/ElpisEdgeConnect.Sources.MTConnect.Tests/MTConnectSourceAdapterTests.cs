...
public sealed class MTConnectSourceAdapterTests : IAsyncDisposable
{
    ...
    [InlineData("mtc-agent")]
    public async Task ValidateConfigAsync_BareHost_ReturnsSuccess(string agentBaseUrl)
    {
        var adapter = CreateAdapter();
        var cfg = CreateConfig(agentBaseUrl: agentBaseUrl);

        var result = await adapter.ValidateConfigAsync(cfg, CancellationToken.None);

        result.IsValid.Should().BeTrue(
            "an operator names an agent by IP or host name; http:// is implied");
    }
